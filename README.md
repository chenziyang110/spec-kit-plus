...
Use the **`/sp.specify`** command to describe what you want to build. Focus on the **what** and **why**, not the tech stack. This command should clarify high-impact ambiguity before it declares the spec ready for planning.
...
| `/sp.specify`      | Align requirements and produce a planning-ready spec plus alignment report |
| `/sp.plan`         | Create technical implementation plans from an aligned spec and its risk state |
...
| Command              | Description                                                                                                                          |
| -------------------- | ------------------------------------------------------------------------------------------------------------------------------------ |
| `/sp.clarify`   | Re-open the spec to clarify, correct, or extend requirements and update alignment status                                               |
...
After this prompt is entered, you should see the agent kick off feature setup, repository scaffolding, and a requirement-alignment loop. It should ask follow-up questions whenever high-impact ambiguity remains instead of silently guessing.

Once this step is completed, you should have a new branch created (e.g., `001-create-taskify`), as well as a new specification in the `specs/001-create-taskify` directory plus an alignment report explaining why the request is ready for planning or why it was force-continued with known risks.

The produced specification should contain bounded user stories, scope boundaries, and functional requirements. The paired alignment report should summarize clarified decisions, low-risk defaults, and any remaining risks.
...
### **STEP 3:** Functional specification clarification and refinement

`/sp.specify` should already do the first serious round of alignment. Use `/sp.clarify` when you need to revisit the spec afterwards: to correct misunderstandings, add new requirements, add technical constraints, or tighten unclear sections.

Run the structured clarification workflow before planning whenever the spec changes materially or still contains meaningful uncertainty.

Preferred order:

1. Use `/sp.clarify` (structured) to repair or extend the spec and update the alignment decision.
2. Optionally follow up with ad-hoc free-form refinement if something still feels vague.

If you intentionally want to force planning despite unresolved risk, state that explicitly so the agent can record `Force proceed with known risks` in the alignment report.
...
It's important to use the interaction with the agent as an opportunity to clarify and challenge the specification. Do not treat its first draft as final, and do not let it stop asking just because the request "sounds clear" from one side.
