...
- Releases publish prebuilt `project-cognition` binaries for Windows, Linux, and macOS; install via `tools/project-cognition/install.sh` or `tools/project-cognition/install.ps1`, or set `PROJECT_COGNITION_BIN` to a custom binary path.
...
- Generated projects require `PROJECT_COGNITION_BIN` or `project-cognition` on PATH before any of these helpers run; the helper scripts prefer `PROJECT_COGNITION_BIN` when set and otherwise call `project-cognition` from PATH.
