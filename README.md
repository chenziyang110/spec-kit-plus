...
- Generated projects use `.specify/project-cognition/status.json` plus the agent-planned task-local project cognition query bundle as the default brownfield runtime truth surface, while `.specify/project-cognition/project-cognition.db` is the canonical graph store.
- New generated workflows use `.specify/project-cognition/status.json`, `.specify/project-cognition/project-cognition.db`, `project-cognition lexicon`, and `project-cognition query --query-plan` as the runtime truth surface. `specify project-map ...` remains a legacy CLI alias for existing projects, but new workflows should not read or require `.specify/project-map/**`.
- Use `map-update` for localized stale cognition runtime refresh and ordinary changed-path maintenance, including adoptable missing path-index coverage. Use `map-scan` followed by `map-build` when the baseline is missing, unusable, schema-incompatible, explicitly being rebuilt, invalidated by broad architecture replacement, or path-index gaps are unadoptable after update classification.
