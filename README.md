...
- When using another local directory as a reference, check whether that directory
  ...
  `alias_index`, `explicit_rebuild_requested`, or `baseline_identity_invalid`.
  `build-from-scan` archives a v1 or incompatible broad-schema DB, migrates a compatible schema v2 DB in place, and publishes schema v3 with typed graph-claim lifecycle tables.
