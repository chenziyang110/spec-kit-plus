...
Use `discussion` before `specify` or `quick` when the idea is exploratory, has product trade-offs, or has unclear context boundaries. `discussion` classifies each turn, answers repository-discoverable facts from live evidence, uses project cognition only as advisory navigation toward minimal live reads, uses deferred persistence for ordinary turns, flushes batched compact events at semantic checkpoints, user-triggered saves, five-turn cadence, compaction risk, or durable lifecycle transitions, and refreshes structured artifacts only at semantic checkpoints. Unsaved replies include a `Compaction Preserve` note when user thinking, decisions, requirement points, feature points, constraints, trade-offs, or reopen conditions must not be lost or reinterpreted. It acts as a senior product-engineering advisor: it performs a Truth Pass before project-specific technical advice, separates verified project facts from assumptions, records advice confidence, gives decision-ready judgment with evidence, risk, recommendation, and next discussion paths, maintains a Discussion Compass during long conversations, uses recommendation-first decision progression, keeps stable response formats, and proactively surfaces adjacent implications instead of forcing one-point-at-a-time replies. Recommendation-first is not questionless: when user judgment is needed, the response asks one explicit primary decision question with the recommended default and meaningful alternatives. It runs a Context Boundary Gate before technicalizing unclear target/reference/external/evidence boundaries, drafts one unified `discussion_requirement_contract` handoff pair only after explicit handoff request and boundary lock, and marks that pair handoff-ready only after self-review and user confirmation. The compatibility filenames remain `handoff-to-specify.md` and `handoff-to-specify.json`, but the pair is one agent-facing requirement contract with `consumer_eligibility` for `sp-specify` and `sp-quick`, `recommended_consumer`, and `quick_task_candidate`; do not generate a second quick-specific handoff. The Markdown handoff includes a `Handoff Reviewer Guide` so an experienced reviewer who does not know Spec Kit internals can approve or request changes against clear criteria. Skills-based projects also install `spec-kit-discussion-handoff-review`, which standardizes review verdicts and applies a ready summary quality check so the final handoff-ready closeout reads like a concise handoff card, not just paths and counters.
...
- `discussion` to shape a rough idea through resumable senior product and technical discussion before formal specification or bounded quick execution. It classifies each turn, answers repository-discoverable facts from live evidence, uses project cognition only as advisory navigation toward minimal live reads, uses deferred persistence for ordinary turns, flushes batched compact events at semantic checkpoints, user-triggered saves, five-turn cadence, compaction risk, or durable lifecycle transitions, and refreshes structured artifacts only at semantic checkpoints. Unsaved replies include a `Compaction Preserve` note when user thinking, decisions, requirement points, feature points, constraints, trade-offs, or reopen conditions must not be lost or reinterpreted. It acts as a senior product-engineering advisor: it performs a Truth Pass before project-specific technical advice, separates verified project facts from assumptions, records advice confidence, gives decision-ready judgment with evidence, risk, recommendation, and next discussion paths, maintains a Discussion Compass during long conversations, uses recommendation-first decision progression, keeps stable response formats, and proactively surfaces adjacent implications instead of forcing one-point-at-a-time replies. Recommendation-first is not questionless: when user judgment is needed, the response asks one explicit primary decision question with the recommended default and meaningful alternatives. It writes `.specify/discussions/<slug>/` artifacts, runs the Context Boundary Gate before technicalizing unclear target/reference/external/evidence boundaries, and creates exactly one draft unified `discussion_requirement_contract` handoff pair: `handoff-to-specify.md` plus `handoff-to-specify.json` only after explicit handoff request and boundary lock. These are compatibility filenames for one agent-facing requirement contract, not a specify-only payload; the handoff includes `consumer_eligibility` for `sp-specify` and `sp-quick`, `recommended_consumer`, `quick_task_candidate`, `handoff_goal`, `context_boundary`, `implementation_target`, `source_evidence`, `blocking_unknowns`, `downstream_instructions`, `quality_gate`, a `Handoff Reviewer Guide`, and a Must-Preserve Ledger. Skills-based projects include `spec-kit-discussion-handoff-review` so reviewers use fixed verdicts and the final ready reply passes a ready summary quality check instead of showing only updated paths and counters. It does not automatically invoke `specify` or `quick`.
...
- `sp-quick` performs one Understanding Checkpoint before substantive execution. The agent presents a Quick Checkpoint card covering issue, target outcome, scope, next action, and completion evidence with concrete details, then waits for confirmation before continuing.
...
- `sp-debug` performs one Debug Understanding Checkpoint before substantive investigation. The agent presents a Debug Checkpoint card covering symptom, expected behavior, investigation scope, first evidence action, and progress signal with concrete details, then waits for confirmation before reproduction, logs, source/test reads, evidence collection, fixes, or validation.
