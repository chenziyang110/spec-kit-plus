...
- When using another local directory as a reference, check whether that directory
  ...
  `alias_index`, `explicit_rebuild_requested`, or `baseline_identity_invalid`.
  Schema v3 is current-only. The current runtime does not migrate schema v2 and does not archive or replace old databases. Remove the incompatible project-cognition.db explicitly, then run `sp-map-scan -> sp-map-build` with the current binary to publish schema v3 with typed graph-claim lifecycle tables.
