...
- Use `map-update` for localized stale cognition refresh recommendations and ordinary changed-path map maintenance; use `map-scan` followed by `map-build` when the user wants a missing, unusable, schema-incompatible, explicitly rebuilt, architecture-replaced, or path-index-incomplete baseline repaired.
...
- Readiness diagnostics such as `recommended_next_action`, `needs_update`, and `needs_rebuild` remain available to explain map quality and suggested map-maintenance follow-up, but ordinary workflows must treat them as advisory unless the user explicitly requested map repair.
...
- `specify`, `clarify`, `deep-research`, `plan`, and `tasks` do not directly rewrite project cognition content; when they discover the current cognition index is too weak or likely outdated for the touched area, they should recommend `map-update` for changed-path map maintenance and let it record partial/low-confidence closure when needed. They should recommend `map-scan` followed by `map-build` when the user wants repair for a missing, unusable, schema-incompatible, explicitly rebuilt, architecture-replaced, or path-index-incomplete baseline.
...
Already have code? Consult `.specify/project-cognition/status.json` and the agent-planned task-local project cognition query bundle first when available. The workflow asks the runtime for a lexicon, the agent expands the user's natural-language request into a `query_plan`, and the runtime executes that plan. For the first brownfield cognition baseline, run `sp-map-scan` followed by `sp-map-build` when you want map maintenance, and require `project-cognition validate-scan --format json` plus `project-cognition validate-build --format json` to pass for that map-maintenance workflow. Use `map-update` as a changed-path and localized stale cognition maintenance follow-up after ordinary work.
...
- Both `sp-fast` and `sp-quick` consult the project cognition advisory layer first: invoke the project launcher from `.specify/config.json` with `project-cognition lexicon --intent implement --query="$ARGUMENTS" --format json`, have the agent translate the raw request into a `query_plan` using returned map terms, then run `project-cognition query --intent implement --query-plan "<query_plan_json>" --format json`. Use the returned readiness, task-local bundle, and `minimal_live_reads` as advisory navigation while proving claims from live repository evidence. Fall back to PATH `specify` only when no project launcher is configured.
...
- First-wave `[AGENT]` coverage started with `sp-fast`, `sp-quick`, `sp-map-scan`, and `sp-map-build`; the shared `specify`, `plan`, `tasks`, `implement`, and `debug` workflows now use the same marker for advisory navigation and required workflow-state updates.
...
Navigation is cognition-first, while technical proof remains live-evidence-first:
...
- During explicit map-maintenance or repair work, invoke the project launcher from `.specify/config.json` for `project-cognition validate-build --format json`, and run `project-cognition complete-refresh --format json` only when build acceptance passes. Fall back to PATH `specify` only when no project launcher is configured.
...
- After a successful map-maintenance refresh, record the new fresh cognition baseline. Dirty metadata is a map-maintenance diagnostic, not an ordinary workflow completion requirement.
- Code changes that alter project cognition meaning should report changed paths and recommend `map-update`; ordinary workflow completion still depends on live code, tests, scripts, configuration, or authoritative docs.
