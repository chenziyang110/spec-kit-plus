...
- `specify`, `clarify`, `deep-research`, `plan`, and `tasks` do not directly rewrite project cognition content; when they discover the current cognition runtime is too weak or likely outdated for the touched area, they should use `map-update` for changed-path refresh. Uncertain closure is recorded by `map-update` as partial/low-confidence facts when needed. They should use `map-scan` followed by `map-build` when the baseline is missing, unusable, schema-incompatible, explicitly being rebuilt, invalidated by broad architecture replacement, or path-index gaps are unadoptable after update classification.
