...
- New generated workflows use `.specify/project-cognition/status.json`, `.specify/project-cognition/project-cognition.db`, `project-cognition lexicon --mode catalog`, and `project-cognition query --query-plan` as advisory navigation inputs. `project-cognition lexicon` returns the schema v2 `alias_index`-backed alias catalog plus graph-backed project concept candidates from the active graph; agents follow the alias-first flow, normalize user input into project vocabulary, write explicit `semantic_intake` with `normalized_query`, `intent_facets`, `negative_constraints`, and `alias_interpretations`, select existing concepts, record rejected concepts in `concept_decisions` with `covered_facets`, `missing_facets`, and `match_sources`, carry `lexicon_generation_id`, and then run `project-cognition query --query-plan`. Facet coverage gates selection; top lexical or vector similarity alone is not route truth. Map points, code proves: the alias catalog is route vocabulary, not evidence by itself. The lexicon payload includes `candidate_universe_version` and `active_generation_id` so query planning stays tied to the graph generation that produced the candidates. When `project-cognition lexicon` returns `agent_normalization.required=true`, agents treat it as a non-intelligent CLI reminder to write `semantic_intake` from the alias catalog; when the field is omitted, treat it as `required=false`, not as proof that raw lexical ranking is authoritative. CJK or mixed CJK/ASCII prompts still require agent-owned translation even if embedded project terms produce positive raw lexical matches. `agent_normalization` is advisory guidance, not a route decision, and the CLI remains a structured tool while the agent owns semantic judgment. `templates/project-map/**` remains a historical compatibility/export surface for handbook generation, but new workflows should not read or require `.specify/project-map/**`.
...
Already have code? Resolve `.specify/project-cognition/status.json` and the agent-planned task-local project cognition query bundle first. The workflow asks `project-cognition lexicon --mode catalog` for the schema v2 `alias_index`-backed alias catalog and graph-backed project concept candidates, the agent follows the alias-first flow, normalizes user input into project vocabulary, writes `semantic_intake` with normalized query, intent facets, negative constraints, and alias interpretations, records `concept_decisions` with selected and rejected concepts plus facet coverage, carries `lexicon_generation_id`, and then calls `project-cognition query --query-plan`; `candidate_universe_version` and `active_generation_id` bind the plan to the candidate graph generation. Map points, code proves: the alias catalog is route vocabulary, not evidence by itself. For the first brownfield cognition baseline, run `sp-map-scan` followed by `sp-map-build`, and require `project-cognition validate-scan --format json` plus `project-cognition validate-build --format json` to pass before downstream work proceeds. If validation reports schema v1, old broad-schema rebuild-required readiness, missing or invalid `alias_index`, or another unusable baseline, `build-from-scan` archives the v1 DB and rebuilds a clean schema v2 database. After that, source-changing `sp-*` workflows run inline project cognition update for their own changed paths and affected surfaces during closeout. Use `map-update` for external/manual localized stale cognition maintenance and ordinary existing-baseline gaps. Use map-scan -> map-build only for first/missing/unusable baseline, schema failure, schema v1 or old broad-schema rebuild-required readiness, zero active-generation path_index rows, missing or invalid alias_index, `explicit_rebuild_requested`, or `baseline_identity_invalid`.
...
- Both `sp-fast` and `sp-quick` still pass the project cognition gate first: run `project-cognition lexicon --intent implement --query="$ARGUMENTS" --mode catalog --format json`, write `semantic_intake`, select from graph-backed project concept candidates, record selected and rejected concepts in `concept_decisions` with `covered_facets`, `missing_facets`, and `match_sources`, carry `lexicon_generation_id`, then run `project-cognition query --intent implement --query-plan "<query_plan_json>" --format json`. The lexicon payload includes `candidate_universe_version` and `active_generation_id`; use them to keep query planning tied to the graph generation that produced the candidates. Generated projects require `PROJECT_COGNITION_BIN` or `project-cognition` on PATH for these helpers; helper scripts prefer `PROJECT_COGNITION_BIN` when set and otherwise call `project-cognition` from PATH. Continue from the returned readiness, task-local bundle, and `minimal_live_reads` before source reads continue.
...
- New generated workflows use `.specify/project-cognition/status.json`, `.specify/project-cognition/project-cognition.db`, `project-cognition lexicon --mode catalog`, and `project-cognition query --query-plan` as advisory navigation inputs. `project-cognition lexicon` returns the schema v2 `alias_index`-backed alias catalog plus graph-backed project concept candidates from the active graph; agents follow the alias-first flow, normalize user input into project vocabulary, write explicit `semantic_intake` with `normalized_query`, `intent_facets`, `negative_constraints`, and `alias_interpretations`, select existing concepts, record rejected concepts in `concept_decisions` with `covered_facets`, `missing_facets`, and `match_sources`, carry `lexicon_generation_id`, and then run `project-cognition query --query-plan`. Facet coverage gates selection; top lexical or vector similarity alone is not route truth. Map points, code proves: the alias catalog is route vocabulary, not evidence by itself. The lexicon payload includes `candidate_universe_version` and `active_generation_id` so query planning stays tied to the graph generation that produced the candidates. `templates/project-map/**` remains a historical compatibility/export surface for handbook generation, but new workflows should not read or require `.specify/project-map/**`.
- Releases publish prebuilt `project-cognition` binaries for Windows, Linux, and macOS. `specify init` best-effort downloads and pins the matching binary in `.specify/config.json`; if that cannot happen, install via `tools/project-cognition/install.sh` or `tools/project-cognition/install.ps1`, or set `PROJECT_COGNITION_BIN` to a custom binary path.
