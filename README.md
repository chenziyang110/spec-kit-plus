...
Schema v4 adds an auditable live feedback loop and a current evidence basis while retaining superseded claim evidence as history. When claim-specific bounded reads settle a stale or contradicted route, create a current-only `claim_reconciliation_contract_version=1` packet with active generation/state preconditions and content hashes, then run `project-cognition claim-reconcile --input <packet.json> --format json` and rerun Compass once. The runtime derives the graph state; callers cannot request a target state, and reconciliation never grants workflow final-claim authorization. Schema v4 is current-only and does not migrate schema v3 or older databases.
...
- When using another local directory as a reference, check whether that directory
  ...
  `alias_index`, `explicit_rebuild_requested`, or `baseline_identity_invalid`.
  Schema v4 is current-only. The current runtime does not migrate schema v3 or older databases and does not archive or replace them. Remove the incompatible project-cognition.db explicitly, then run `sp-map-scan -> sp-map-build` with the current binary to publish schema v4 with typed graph-claim lifecycle and reconciliation tables.
