...
class TestAgentConfigConsistency:
    ...
    def test_get_skills_dir_uses_agent_config_for_moved_skill_agents(self, tmp_path):
        ...
        assert _get_skills_dir(tmp_path, "grok") == tmp_path / ".grok" / "skills"
    ...
    def test_runtime_config_paths_for_nonstandard_agents(self):
        ...
        assert AGENT_CONFIG["grok"]["requires_cli"] is True
        assert AGENT_CONFIG["grok"]["install_url"] == "https://x.ai/cli"
    ...
    def test_extension_registrar_configs_for_nonstandard_agents(self):
        ...
        expected = {
            ...
            "grok": {"dir": ".grok/skills", "extension": "/SKILL.md"},
    ...
    def test_grok_agent_context_scripts_use_root_agents_file(self):
        """Grok context updates should target root AGENTS.md while skills live under .grok."""
        bash_text = (REPO_ROOT / "scripts" / "bash" / "update-agent-context.sh").read_text(encoding="utf-8")
        pwsh_text = (REPO_ROOT / "scripts" / "powershell" / "update-agent-context.ps1").read_text(encoding="utf-8")
        bash_wrapper = (
            REPO_ROOT / "src" / "specify_cli" / "integrations" / "grok" / "scripts" / "update-context.sh"
        ).read_text(encoding="utf-8")
        pwsh_wrapper = (
            REPO_ROOT / "src" / "specify_cli" / "integrations" / "grok" / "scripts" / "update-context.ps1"
        ).read_text(encoding="utf-8")

        assert 'GROK_FILE="$AGENTS_FILE"' in _compact_assignment_text(bash_text)
        assert "$GROK_FILE=Join-Path$REPO_ROOT'AGENTS.md'" in _compact_assignment_text(pwsh_text)
        assert "Grok" in bash_text
        assert "Grok" in pwsh_text
        assert "AGENTS.md" in bash_wrapper
        assert "AGENTS.md" in pwsh_wrapper
        assert "grok" in bash_wrapper
        assert "grok" in pwsh_wrapper
    ...
    def test_powershell_validate_set_includes_nonstandard_agents(self):
        ...
        for agent_key in ("kimi", "trae", "pi", "zcode", "grok"):
    ...
    def test_agent_context_scripts_include_simple_nonstandard_agents(self):
        ...
        assert "grok" in bash_text
        assert "GROK_FILE" in bash_text
        assert "grok" in pwsh_text
        assert "GROK_FILE" in pwsh_text

    def test_ai_help_includes_nonstandard_agents(self):
        """CLI help text for --ai should include nonstandard agent keys."""
        for agent_key in ("tabnine", "kimi", "trae", "pi", "iflow", "mimo", "zcode", "grok"):
