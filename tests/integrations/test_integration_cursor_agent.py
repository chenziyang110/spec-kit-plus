...
def test_cursor_runtime_skills_hard_gate_project_cognition_reads(tmp_path):
    ...
    for rel in (
        ...
        expected = next(value for key, value in canonical.items() if key in rel)
        assert f"closeout-plan --workflow {expected}" in content
        assert "update_argv" in content
        assert "explicit workflow-owned" in content
        assert "update --payload-file" not in content
        assert "update --delta-session" not in content
        assert "update --changed-path" not in content
        ...
        if "sp-implement" in rel:
            ...
            assert "query --query-plan" not in content
        else:
...
def test_cursor_closeout_advisory_uses_exact_heading_marker(tmp_path):
    ...
    assert "Entry advisory is not closeout ownership" in content
    assert "follow the rendered planner-first closeout contract" in content.lower()
    assert "registry-owned literal `sp-*` workflow ID" in content
    assert "update_argv" in content
    assert "update --payload-file" not in content
    assert "update --delta-session" not in content
    assert "update --changed-path" not in content
