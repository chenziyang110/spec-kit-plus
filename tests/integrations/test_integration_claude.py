...
def _load_claude_hook_dispatch_module():
    ...
    spec = importlib.util.spec_from_file_location(
        "claude_hook_dispatch_for_tests", hook_path
    )
...
def _load_shared_hook_launcher_module():
    ...
    spec = importlib.util.spec_from_file_location(
        "specify_hook_launcher_for_tests", hook_path
    )
    assert spec is not None
    module = importlib.util.module_from_spec(spec)
    assert spec.loader is not None
...
def test_shared_native_hook_launcher_timeout_fails_open(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.setattr(
        module,
        "_dispatch_script",
        lambda _project_root, _integration_key: dispatch_script,
    )
    monkeypatch.setattr(
        module.sys, "argv", ["specify-hook.py", "claude", "session-start"]
    )
...
def test_claude_hook_suppresses_optional_recovery_summary_next_action(
    tmp_path, monkeypatch
):
    module = _load_claude_hook_dispatch_module()
    context = {
...
def test_claude_stop_monitor_suppresses_optional_checkpoint_resume_cue(
    tmp_path, monkeypatch
):
    module = _load_claude_hook_dispatch_module()
    context = {
        "command_name": "quick",
        "workspace": str(tmp_path / ".planning" / "quick" / "001-demo"),
    }
    ...
    monkeypatch.setattr(module, "_run_shared_hook", fake_run_shared_hook)
    monkeypatch.setattr(
        module, "_learning_signal_context", lambda *_args, **_kwargs: ""
    )

    payload = module._handle_stop_monitor(tmp_path, {})

    assert payload == {
        "systemMessage": "checkpoint recommended before further work continues"
    }
...
class TestClaudeIntegration:
    ...
    def _expected_inventory(cls, script_variant: str) -> list[str]:
        ...
        for stem in cls._command_stems():
            ...
            for reference in claude.list_command_reference_templates(stem):
                ...
                expected.append(f"{skills_prefix}/sp-{stem}/references/{rel_reference}")
    ...
    def test_feature_creation_surfaces_use_explicit_helper_paths_without_fake_cli(
        self, tmp_path
    ):
        integration = get_integration("claude")
        manifest = IntegrationManifest("claude", tmp_path)
        ...
        for path in surfaces:
            ...
            assert ".specify/scripts/bash/create-new-feature.sh" in content, (
                f"{path} missing bash helper path"
            )
            assert ".specify/scripts/powershell/create-new-feature.ps1" in content, (
                f"{path} missing powershell helper path"
            )
            assert "run `specify create-feature`" not in content, (
                f"{path} teaches fake runnable create-feature command"
            )
    ...
    def test_setup_writes_shell_free_hook_entries_for_powershell_script_variant(
        self, tmp_path
    ):
        integration = get_integration("claude")
        manifest = IntegrationManifest("claude", tmp_path)
    ...
    def test_teardown_strips_current_node_command_hooks_from_user_settings(
        self, tmp_path
    ):
        integration = get_integration("claude")
        claude_dir = tmp_path / ".claude"
            ...
            json.dumps(
                {
                    ...
                    "hooks": {
                        "SessionStart": [
                            {
                                "hooks": [
                                    self._expected_launcher_hook(
                                        "session-start", script_type="sh"
                                    )
                                ]
                            }
                        ]
    ...
    def test_claude_hook_adapter_respects_event_specific_output_schema(self):
        ...
        pre_tool = module._shared_to_claude_output(
            hook_event_name="PreToolUse", shared_payload=blocked
        )
        assert pre_tool == {
            "hookSpecificOutput": {
        ...
        prompt = module._shared_to_claude_output(
            hook_event_name="UserPromptSubmit", shared_payload=blocked
        )
        assert prompt["decision"] == "block"
        assert prompt["reason"] == "blocked reason"
        assert "hookSpecificOutput" not in prompt

        post_tool = module._shared_to_claude_output(
            hook_event_name="PostToolUse", shared_payload=blocked
        )
        ...
        detailed = module._shared_to_claude_output(
            hook_event_name="UserPromptSubmit",
            shared_payload=detailed_block,
        )
        assert "Workflow/stage: spx-implement / protected-ci" in detailed["reason"]
        assert "Steps: 1. Approve: Click verify" in detailed["reason"]
        assert "Return: pipeline URL" in detailed["reason"]
        assert "Human resume: Resume spx-implement" in detailed["reason"]

        pre_tool_warning = module._shared_to_claude_output(
            hook_event_name="PreToolUse", shared_payload=warned
        )
        assert pre_tool_warning == {"systemMessage": "warning context"}

        assert module._stop_system_message("stop context") == {
            "systemMessage": "stop context"
        }
    ...
    def test_claude_hook_dispatch_blocks_bypass_prompt_via_shared_engine(
        self, tmp_path
    ):
        integration = get_integration("claude")
        manifest = IntegrationManifest("claude", tmp_path)
    ...
    def test_claude_hook_dispatch_blocks_sensitive_read_via_shared_engine(
        self, tmp_path
    ):
        integration = get_integration("claude")
        manifest = IntegrationManifest("claude", tmp_path)
    ...
    def test_claude_hook_dispatch_does_not_block_repairable_prompt_entry(
        self, tmp_path
    ):
        integration = get_integration("claude")
        manifest = IntegrationManifest("claude", tmp_path)
    ...
    def test_claude_hook_dispatch_allows_repairable_state_read(self, tmp_path):
        ...
        result = subprocess.run(
            [sys.executable, str(hook_script), "pre-tool-read"],
            input=json.dumps(
                {"tool_name": "Read", "tool_input": {"file_path": str(state_path)}}
            ),
            text=True,
            capture_output=True,
            check=False,
    ...
    def test_claude_hook_dispatch_allows_repairable_validate_state_autofix_bash(
        self, tmp_path
    ):
        integration = get_integration("claude")
        manifest = IntegrationManifest("claude", tmp_path)
    ...
    def test_claude_hook_dispatch_allows_repairable_workflow_state_write(
        self, tmp_path
    ):
        integration = get_integration("claude")
        manifest = IntegrationManifest("claude", tmp_path)
    ...
    def test_claude_hook_dispatch_allows_repairable_implement_tracker_write(
        self, tmp_path
    ):
        integration = get_integration("claude")
        manifest = IntegrationManifest("claude", tmp_path)
    ...
    def test_claude_hook_dispatch_allows_repairable_implement_tracker_read(
        self, tmp_path
    ):
        integration = get_integration("claude")
        manifest = IntegrationManifest("claude", tmp_path)
    ...
    def test_claude_shared_hook_fallbacks_share_one_event_deadline(
    ...
    def test_claude_shared_hook_client_maps_blocked_payload(
        self, tmp_path, monkeypatch
    ):
        module = _load_claude_hook_dispatch_module()
        monkeypatch.setattr(
    ...
    def test_claude_shared_hook_client_rejects_exit_10_with_ok_payload(
        self, tmp_path, monkeypatch
    ):
        module = _load_claude_hook_dispatch_module()
        monkeypatch.setattr(
    ...
    def test_claude_shared_hook_client_redacts_invalid_output(
        self, tmp_path, monkeypatch
    ):
        module = _load_claude_hook_dispatch_module()
        monkeypatch.setattr(
    ...
    def test_claude_hook_dispatch_blocks_when_project_launcher_is_invalid(
        self, tmp_path
    ):
        integration = get_integration("claude")
        manifest = IntegrationManifest("claude", tmp_path)
    ...
    def test_claude_hook_dispatch_adds_statusline_context_on_session_start(
        self, tmp_path
    ):
        integration = get_integration("claude")
        manifest = IntegrationManifest("claude", tmp_path)
    ...
    def test_claude_hook_dispatch_surfaces_learning_signal_on_post_tool_use(
        self, tmp_path
    ):
        integration = get_integration("claude")
        manifest = IntegrationManifest("claude", tmp_path)
        ...
        (feature_dir / "workflow-state.md").write_text(
            "\n".join(
                [
                    ...
                    "## Learning Triggers",
                    "",
                    "- user_correction: sp-plan must stop before tasks.md",
                    "",
                    "## False Starts",
                    "",
    ...
    def test_claude_hook_dispatch_blocks_stop_when_checkpoint_state_is_missing(
        self, tmp_path
    ):
        integration = get_integration("claude")
        manifest = IntegrationManifest("claude", tmp_path)
    ...
    def test_claude_hook_dispatch_surfaces_stop_checkpoint_warning_as_system_message(
        self, tmp_path
    ):
        integration = get_integration("claude")
        manifest = IntegrationManifest("claude", tmp_path)
        ...
        payload = json.loads(result.stdout.strip())
        assert "hookSpecificOutput" not in payload
        assert payload["systemMessage"].startswith(
            "checkpoint recommended before further work continues"
        )
        assert (
    ...
    def test_claude_hook_dispatch_allows_specify_prompt_with_legacy_bootstrap_state(
        self, tmp_path
    ):
        integration = get_integration("claude")
        manifest = IntegrationManifest("claude", tmp_path)
    ...
    def test_claude_hook_dispatch_reads_compaction_resume_context_on_session_start(
        self, tmp_path
    ):
        integration = get_integration("claude")
        manifest = IntegrationManifest("claude", tmp_path)
    ...
    def test_uninstall_preserves_user_settings_while_removing_managed_hooks(
        self, tmp_path
    ):
        integration = get_integration("claude")
        manifest = IntegrationManifest("claude", tmp_path)
        ...
        assert not any(
            "claude-hook-dispatch.py" in command for command in remaining_commands
        )
        assert (tmp_path / ".claude" / "hooks" / "claude-hook-dispatch.py") in removed
        assert skipped == [] or settings_path in skipped

    def test_setup_does_not_duplicate_managed_hook_entries_on_repeat_install(
        self, tmp_path
    ):
        integration = get_integration("claude")
        manifest = IntegrationManifest("claude", tmp_path)
    ...
    def test_uninstall_removes_install_owned_settings_json_when_only_managed_hooks_exist(
        self, tmp_path
    ):
        integration = get_integration("claude")
        manifest = IntegrationManifest("claude", tmp_path)
    ...
    def test_init_generates_claude_hooks_as_command_strings_without_args(
        self, tmp_path
    ):
        from typer.testing import CliRunner
        from specify_cli import app
    ...
    def test_ai_flag_auto_promotes_and_enables_skills(self, tmp_path):
        ...
        assert (
            project / ".claude" / "skills" / "spec-kit-workflow-routing" / "SKILL.md"
        ).exists()
        assert (
            project
            / ".claude"
        ...
        assert not (
            project / ".claude" / "skills" / "spec-kit-project-map-gate"
        ).exists()
...
def test_claude_generated_runtime_facing_skills_include_native_subagent_contract(
    tmp_path,
):
    from typer.testing import CliRunner
    from specify_cli import app
...
def test_claude_generated_implement_skill_includes_shared_leader_gate(tmp_path):
    ...
    assert (
        "multiple validated lanes with isolated write sets and an explicit join point"
        in content
    )
    assert "dispatch only from validated `workertaskpacket`" in content
    assert (
        "compile and validate a `workertaskpacket` just in time only for delegated work"
        in content
    )
    ...
    assert (
        "attempt native subagent execution before leader-inline fallback" not in content
    )
    assert (
        "concrete fallback reason in `feature_dir/implement-tracker.md`" not in content
    )
...
def test_claude_generated_sp_implement_description_prefers_subagent_dispatch(tmp_path):
    ...
    content = (target / ".claude" / "skills" / "sp-implement" / "SKILL.md").read_text(
        encoding="utf-8"
    )
    parts = content.split("---", 2)
    parsed = yaml.safe_load(parts[1])
...
def test_claude_generated_sp_implement_teams_skill_uses_agent_teams_surface(tmp_path):
    ...
    assert (
        "the first non-prerequisite action is creating or resuming the claude agent team"
        in lower
    )
    assert (
        "do not read `plan.md`, `tasks.md` beyond the minimum existence/status check"
        in lower
    )
    assert (
        "do not run validation, edit files, or inspect broad implementation context before this gate passes"
        in lower
    )
    ...
    assert (
        "if the first `teamcreate` / agent teams call fails as though the feature is disabled"
        in lower
    )
    assert (
        "explicitly remind the user to enable agent teams in claude code settings or environment"
        in lower
    )
    ...
    assert (
        "create the full task set before wiring `blockedby` / `blocks` dependencies"
        in lower
    )
    ...
    assert (
        "if a team for the same feature slug is already active, reuse or resume it"
        in lower
    )
    assert "do not create a second parallel team for the same feature" in lower
    assert (
        "after each completed join point or ready batch, immediately re-read the shared task ledger"
        in lower
    )
...
def test_claude_generated_skills_preserve_agent_required_marker_lines(tmp_path):
    ...
    for skill_name in (
        ...
        content = (target / ".claude" / "skills" / skill_name / "SKILL.md").read_text(
            encoding="utf-8"
        )
        assert "[AGENT]" in content
