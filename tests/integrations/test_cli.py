...
class TestInitIntegrationFlag:
    ...
    def test_codex_init_advertises_sp_teams_surface(self, tmp_path):
        ...
        assert "sp-integrate" not in result.output.lower()
        assert "/sp.integrate" not in result.output.lower()
...
def test_check_reports_workflow_contract_drift(tmp_path):
    ...
    (project / ".specify" / "templates" / "commands" / "analyze.md").write_text(
        "Run specify-runtime lane resolve --command analyze --format json\n",
        encoding="utf-8",
    )
    ...
    assert result.exit_code == 0, result.output
    lowered = " ".join(result.output.lower().split())
    assert "retired feature lane resolver" in lowered
