...
def test_update_agent_context_managed_block_uses_refresh_or_dirty_binary_and_memory_semantics() -> None:
    ...
    for content in (bash, powershell):
        ...
        assert "specify project-cognition complete-refresh --format json" in content
        assert 'specify project-cognition mark-dirty --reason "<reason>" --format json' in content
