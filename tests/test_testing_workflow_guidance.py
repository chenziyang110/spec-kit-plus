...
def test_implement_contract_inputs_are_binding_for_execution_and_workers():
    ...
    assert "required when present" in implement_content
    assert "binding execution fields" in implement_content
    assert "execution contract inputs" in worker_prompt
    assert "needs_context" in worker_prompt
    assert "return `needs_context` or `blocked`; do not guess" in worker_prompt
