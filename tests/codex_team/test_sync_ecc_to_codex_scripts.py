...
def test_sync_ecc_to_codex_shell_wrapper_exists_and_avoids_omx_setup_surface():
    script = Path("scripts/sync-ecc-to-codex.sh")
    assert script.exists()

    content = script.read_text(encoding="utf-8")
    assert "omx setup" not in content
    assert "dist/cli/index.js" in content
    assert "--dry-run" in content


def test_sync_ecc_to_codex_powershell_wrapper_exists_and_avoids_omx_setup_surface():
...
def test_readme_documents_how_to_refresh_codex_config_after_mcp_install():
    ...
    assert (
        'uv tool install --force "specify-cli[mcp] @ '
        'git+https://github.com/chenziyang110/spec-kit-plus.git"'
    ) in readme
    assert 'pip install "specify-cli[mcp]"' not in readme
