...
- [ ] **FND-04**: Implement a "No Fix Without Proof" gate: The agent must verify the bug with a reproduction script or failing test before attempting a fix.

### Context Awareness (CTX)
- [ ] **CTX-01**: Automatically load `constitution.md`, `spec.md`, `plan.md`, and `tasks.md` during the investigation phase to provide system-level context.
- [ ] **CTX-02**: Identify "Recently Modified Files" by analyzing git history or Spec Kit task logs to prioritize the search space.
- [ ] **CTX-03**: Integrate with `ROADMAP.md` to understand which feature phase the current bug belongs to.

### Systematic Investigation (SYS)
- [ ] **SYS-01**: "Gather" Node: Ask clarifying questions to capture symptoms (Expected vs Actual) and reproduction steps.
- [ ] **SYS-02**: "Investigate" Node: Formulate and test hypotheses one by one, recording "Evidence" and "Eliminated Theories" in the debug file.
