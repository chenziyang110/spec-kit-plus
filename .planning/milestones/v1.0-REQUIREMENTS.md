# Milestone v1.0 Requirements Archive: Debug Workflow
...
- [x] **FND-02**: Implement a Markdown-based persistence handler that serializes the current graph state, hypothesis, and evidence to `.planning/debug/[slug].md`.
- [x] **FND-03**: Support "Resume Mode": When `sp-debug` is called without arguments, it automatically detects the most recent unfinished session from `.planning/debug/` and restores its full state.
...
- [x] **SYS-03**: "Fix" Node: Propose and apply minimal changes based on the confirmed root cause.
- [x] **SYS-04**: "Verify" Node: Re-run reproduction scripts and phase-level tests to prove the fix works and no regressions were introduced.

### Tools & Integration (TOL)
- [x] **TOL-01**: Provide `sp-debug` with tools to read files, search for symbols, run terminal commands, and edit code.
- [x] **TOL-02**: Implement a Human-in-the-Loop (HITL) checkpoint when verification fails or a decision requires user confirmation.
- [x] **TOL-03**: Register the `/sp-debug` command in the `specify` CLI.
...
| TOL-01 | Phase 3 | Complete |
| TOL-02 | Phase 3 | Complete |
| TOL-03 | Phase 1 | Complete |
...
- [ ] **OPT-02**: Multi-Model Routing: Use cheaper models for gathering/summarization and expensive models for core investigation.
...
---
*Last updated: 2026-04-13 after Phase 3 verification*
