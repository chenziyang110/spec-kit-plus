...
- After a successful `sp-map-update`, committing the refreshed source changes does not require a full rebuild by itself. Ordinary `ready`/`no_op` closeout uses receipt-bound `project-cognition complete-refresh`; until it succeeds the runtime gate withholds Compass/query as `update_finalization_pending`. `project-cognition record-refresh` is only a compatibility or explicit metadata-recording entrypoint when a matching validation receipt already exists, and cannot bypass that closeout branch. Rebuild only when validation reports `needs_rebuild`.
