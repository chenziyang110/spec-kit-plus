...
- **Advisory project cognition index**:
  ...
  - Schema v3 is current-only. The current runtime does not migrate schema v2 and does not archive or replace old databases. Remove the incompatible project-cognition.db explicitly, then run `sp-map-scan -> sp-map-build` with the current binary.
