...
- **Post-implementation system review and human acceptance**: Implement closeout prepares `implementation-handoff.json`. `sp-review` starts the integrated product, exercises required user journeys, repairs bounded startup/interaction/wiring defects, and binds typed evidence to the current snapshot. Review closeout refreshes the final summary and prepares fingerprinted `human-acceptance.json`; `sp-accept` guides the human one observable step at a time and returns product defects to Review without editing source.
