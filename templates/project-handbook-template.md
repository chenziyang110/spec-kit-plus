...
Schema v4 adds an auditable live feedback loop and a current evidence basis while preserving superseded claim evidence as history. Decisive claim-specific bounded reads use a current-only `claim_reconciliation_contract_version=1` packet and `project-cognition claim-reconcile --input <packet.json> --format json`; the runtime validates active generation/state plus file hashes and derives the graph lifecycle state. Ready output says to rerun Compass once, but remains route-candidate evidence and cannot authorize a workflow final claim. Schema v4 does not migrate schema v3 or older databases.
...
- **Agent-native phase pipeline**: `handoff-to-specify.json` -> `spec-contract.json` -> `plan-contract.json` -> `task-index.json` -> per-task lifecycle records. Generate project-facing Markdown and research/design/lane artifacts only when triggered. Compile delegated WorkerTaskPackets just in time and review implementation on drift, joins, validation failure, obligation conflict, or review-window limits rather than after every batch.
...
- **Advisory project cognition index**:
  ...
  - Schema v4 is current-only. The current runtime does not migrate schema v3 or older databases and does not archive or replace them. Remove the incompatible project-cognition.db explicitly, then run `sp-map-scan -> sp-map-build` with the current binary.
