...
- **Post-implementation system review and human acceptance**: Implement closeout prepares `implementation-handoff.json` with official entrypoints, system scenarios, and review obligations. In `sp-review`, the Leader freezes the Review Universe, dispatches independent read-only Audit subagents, joins every result, sends accepted findings to isolated Fix subagents, and assigns different subagents for revalidation. Approved-scope defects—including missing code, task omissions, large repairs, and unknown root causes—remain owned by Review; only a proven requirement, design, or architecture truth gap may return upstream. Approval requires zero uncovered obligations/surfaces, all packets joined, fresh snapshot-bound evidence, and no blocking finding. Review closeout refreshes the final summary and prepares fingerprinted `human-acceptance.json`; `sp-accept` guides the human one observable step at a time and returns product/runtime defects to Review without editing source.
