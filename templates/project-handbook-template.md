...
- **Quick workflow confirmation**: `sp-quick` presents a plain-text Quick Checkpoint before substantive execution. The card must use the fixed Markdown table with `| Item | Current understanding |` and rows for issue, target outcome, boundaries, known facts/assumptions, affected surfaces, concrete implementation plan, next action, validation evidence, and stop condition, then waits for confirmation before implementation, delegation, broad repository analysis, or validation commands. Freeform prose or bullet-only confirmations do not satisfy this gate. Checkpoint cards must not use HTML tags or inline line-break markup in terminal output.
