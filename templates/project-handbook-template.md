...
- **Discussion handoff assessment previews**: When a discussion looks mature enough for handoff but the user has not explicitly requested handoff, the visible stage is `handoff-assessment-preview`. Give the likely verdict, proposed handoff goal, recommended consumer, package scope, excluded scope, readiness checks, default next action, and override path without writing or claiming `handoff-assessment.md`.
- **Discussion next-step content**: When `sp-discussion` recommends a default next step, the same reply should include the first-pass content for that step, such as a draft, option board, release closeout board, handoff assessment checklist, evidence plan, or field-by-field responsibility audit table. Do not make the user ask for "next" just to see the content already recommended.
