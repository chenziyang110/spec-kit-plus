...
- Compass returns `recommended_next_action` as an object. Read its `action_id`; for `needs_rebuild`, report `rebuild_reasons[]` and follow the installed profile's canonical `workflow_routes` steps (`spx-map-rebuild` for Advanced, or `sp-map-scan` then `sp-map-build` for Classic). Do not guess a rebuild cause or convert the legacy runtime status string into a workflow name.
