...
- [For each capability, state why it matters and which topical file should be
  read next.]
- **Project Learning lifecycle**: Agents consume reusable project Learning only through the project runtime's read-only progressive disclosure: `specify-runtime learning start --command <workflow> --format json`, filtered/paginated `specify-runtime learning list`, then one selected `specify-runtime learning show --ref <ref>`. Production is separate: `specify-runtime learning capture` / `capture-auto` create or merge candidates and explicit `specify-runtime learning promote` changes lifecycle state. Classic commands use the shared Learning partial/passive skill; Advanced skills use `references/project-learning.md`; storage files are runtime implementation details rather than normal agent read surfaces.
...
- **Fixed workflow artifact runtime**: Read canonical Agent artifacts progressively with `specify-runtime artifact show --path <project-relative-path> --view summary`, expanding to `--view full` only when needed. Authorized replacement uses one lease from `artifact prepare` followed by `artifact submit`. `specify-runtime artifact catalog` lists deterministic scaffold kinds, and `specify-runtime artifact scaffold --kind <plan-contract|quick-status> --path <project-relative-path> --vars <compact-json>` creates registered boilerplate create-only with blocked/draft readiness defaults and explicit semantic fill targets. Source code and tests remain normal task-scoped edits.
...
- **Cross-project cognition reference**: use the project cognition runtime as
  ...
  `specify-runtime cognition discover --root <path> --format json`, and use that project's
...
- **Runtime truth**: `.specify/project-cognition/status.json`, `.specify/project-cognition/project-cognition.db`, the task-local `specify-runtime cognition compass` packet, and conditional advanced project cognition query bundles
