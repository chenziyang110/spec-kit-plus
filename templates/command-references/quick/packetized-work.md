...
- When the local CLI is available and no runtime-managed result channel exists, prefer `{{specify-subcmd:specify-runtime result path --command quick --workspace .planning/quick/<id>-<slug> --lane-id <lane-id>}}` to compute the canonical handoff target and `{{specify-subcmd:specify-runtime result submit --command quick --workspace .planning/quick/<id>-<slug> --lane-id <lane-id> --result-file <path>}}` to normalize and write the subagent result envelope. `result path` emits JSON and does not accept `--format`; do not append `--format`.
