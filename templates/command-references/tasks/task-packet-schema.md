...
- Supply only task-shaping fields to `specify-runtime tasks`: objective, dependencies, expected write scope, required refs, acceptance, verification, optional cheap `task_checks`, obligation refs, join point, and packet mode. The CLI owns `task-index.json`.
