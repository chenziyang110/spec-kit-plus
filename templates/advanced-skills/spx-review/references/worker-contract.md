...
These are orchestration waves inside the validation ledger shared across
Implement and Review, not separate permission to rerun heavyweight gates. Do not
reset the ledger. The Leader opens attempts inside Review's delivery gate.
Read-only workers may observe slices within an open attempt but cannot open
their own. The combined flow has three logical gates; interruptions retry inside
delivery and real failures require repair plus a new fingerprint.
...
After all repair joins, open a new attempt in the delivery gate and run an
