...
`{{specify-subcmd:sp-teams doctor}}` before the first batch and
`{{specify-subcmd:sp-teams live-probe}}` after install/repair or uncertain
health. Resume the existing feature session; never start a second leader.

Use `{{specify-subcmd:sp-teams auto-dispatch --feature-dir <feature-dir>}}` for
...
`{{specify-subcmd:sp-teams result-template --request-id <id>}}` or inspect
`{{specify-subcmd:sp-teams submit-result --print-schema}}`; a pending template
is not a completed result.

Await every request and reject missing, stale, out-of-scope, or unverified
results. Run `{{specify-subcmd:sp-teams complete-batch --batch-id <id>}}` only
after terminal results and accepted joins. Use
`{{specify-subcmd:sp-teams sync-back}}` before leader-visible verification when
