...
Inspect `.specify/memory/constitution.md` through `specify-runtime artifact show`, alongside the consume-only Learning CLI intake,
the project handbook, and cognition-selected live evidence needed to understand
current practice. Query active feature `spec.md`, `plan.md`, `tasks.md`, and
`workflow-state.md` through targeted `specify-runtime artifact show` calls; use
these artifacts without editing them so the highest affected downstream
...
If missing, create `.specify/memory/constitution.md` with `specify-runtime artifact scaffold --kind constitution --path .specify/memory/constitution.md`; revise it only through fresh leased patches: `--heading`/`--new-heading` for the document title, `--preamble` for the compact Sync Impact Report, and `--section`/`--content` for changed semantic sections. Never submit or reconstruct the whole document. Use the installed
...
complete Sync Impact Report. Then run
`{{specify-subcmd:specify-runtime hook validate-artifacts --command constitution --feature-dir .specify/memory --format json}}`
and refuse completion until the Go gate accepts the version/date metadata and
principle structure. This invocation authorizes only this workflow
