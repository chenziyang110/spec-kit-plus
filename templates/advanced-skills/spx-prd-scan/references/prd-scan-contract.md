...
acceptance, verification, and a runtime-managed inline result channel. Submit
results through `specify-runtime result submit --command prd-scan --result-json`
(or the integration's team result CLI), never by writing a result path. Account for every
