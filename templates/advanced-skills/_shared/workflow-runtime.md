...
`FEATURE_DIR/workflow.json` file.
...
`{{specify-subcmd:specify-runtime workflow enter --command specify --feature-dir <feature-dir> --format json}}`.
When entering any later required stage, run
`{{specify-subcmd:specify-runtime workflow transition --to <this-stage> --feature-dir <feature-dir> --expected-revision <revision> --format json}}`
...
acceptance must not reverse `specify-runtime workflow transition` or edit phase state. Route its
...
`{{specify-subcmd:specify-runtime workflow complete-stage --feature-dir <feature-dir> --expected-revision <revision> --format json}}`.
...
`{{specify-subcmd:specify-runtime workflow next --feature-dir <feature-dir> --format json}}`
...
`specify-runtime workflow resolve`, preserves the prior blocker audit, and reactivates the same
stage.
...
`next_argv` verbatim. That revision-bound argv invokes `specify-runtime workflow closeout`; do
