...
```bash
specify-runtime cognition semantic-audit-resume --format json < resume-validation.json
```
...
```bash
specify-runtime cognition semantic-audit-resume --format json < resume-validation-route-changed.json
```
