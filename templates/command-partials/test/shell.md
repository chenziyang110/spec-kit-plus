...
- Primary inputs are user intent, existing `.specify/testing/` artifacts, the project cognition runtime (`status.json`, `slices/change.json`, and targeted live evidence), and any project memory files.
...
- Report exactly one next command: `/sp-test-scan`, `/sp-test-build`, `/sp-map-update`, or `/sp-map-scan` followed by `/sp-map-build`.
