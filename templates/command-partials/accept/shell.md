...
- Query the reviewed `implementation-summary.md` and `human-acceptance.json` through `specify-runtime artifact show`, then use the frozen Human Acceptance Universe and real user entrypoint. Each selected requirement must retain a stable obligation/scenario mapping; require zero uncovered required obligations.
...
- Allowed: acceptance-owned fields in runtime-managed `human-acceptance.json`, written through the launcher-bound `accept` CLI subcommands; acceptance-owned rich resume/evidence in existing `workflow-state.md`, written only through targeted `artifact show` plus a fresh leased `artifact patch` per changed section. Acceptance never submits or reconstructs the whole state document and never authors `workflow.json`; only `specify-runtime workflow` changes that compact phase lock during route-repair or terminal closeout.
