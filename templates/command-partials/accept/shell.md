...
- Transition from the validated `review` stage into `accept` through the CLI-owned workflow runtime before any acceptance-owned write; the runtime state is the required phase lock.
...
- Allowed: acceptance-owned fields in the runtime-managed `human-acceptance.json` session artifact and workflow state, written through the `accept` and `workflow` CLI commands.
