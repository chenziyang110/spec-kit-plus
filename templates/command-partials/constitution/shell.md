...
- If absent, create `.specify/memory/constitution.md` with `specify-runtime artifact scaffold --kind constitution --path .specify/memory/constitution.md`. Otherwise query it through `artifact show`; patch only the changed title, preamble, or named sections through fresh leases. Never submit or reconstruct the whole constitution.
...
- Build only changed semantic sections and the compact sync-impact report in memory. Use leased `artifact patch --heading`/`--new-heading` for the title, `--preamble` for the report, and `--section`/`--content` for body changes.
