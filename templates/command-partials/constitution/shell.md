...
- If the repository already has code and you need repo-derived evidence, use `BUILD-HANDBOOK.md` and `.specify/project-map/index/status.json` to decide whether the runtime handbook system is fresh enough to trust. If the runtime handbook system is missing or stale for an existing codebase, run `/sp-map-scan` followed by `/sp-map-build` before continuing or explicitly report the refresh as a blocking follow-up.
