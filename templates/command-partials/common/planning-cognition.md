...
- When claim-specific bounded reads settle a stale or contradicted route, create a current-only `project-cognition claim-reconcile --input <packet.json> --format json` packet with `claim_reconciliation_contract_version: 1`, `expected_generation_id`, workflow, RFC3339 `observed_at`, stable `claim_id`, `expected_state`, reason, and evidence carrying `source_path`, `span`, role, and `expected_content_hash`. The runtime re-reads the files and the runtime derives lifecycle state; never send `requested_state`. Generic verification cannot re-promote a graph claim. If reconciliation is ready, rerun Compass once for the planning route; otherwise withhold the claim.
