...
For a selected stale or contradicted claim, open only the returned claim-specific bounded live reads. If those reads are decisive, write a current-only packet with `claim_reconciliation_contract_version: 1`, the packet's `expected_generation_id`, workflow, RFC3339 `observed_at`, and one item per stable claim ID. Each item carries `expected_state`, reason, bounded evidence with repository-relative `source_path`, `span`, role, and `expected_content_hash`, plus optional claim-specific verification. Run `project-cognition claim-reconcile --input <packet.json> --format json`; the runtime re-reads non-ignored files, validates hashes and active-generation/state preconditions, and the runtime derives the target state—never submit `requested_state` or another target-state field. A generic workflow verification is insufficient. On `result_state=ready`, rerun Compass once and use the new packet only for routing; on partial or blocked output, withhold the claim and follow `recommended_next_action`.
