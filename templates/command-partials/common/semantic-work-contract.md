...
- **Graph reconciliation is not WorkContract authorization**: `project-cognition claim-reconcile` consumes claim-specific bounded live evidence under `claim_reconciliation_contract_version: 1` with `expected_generation_id`, `expected_state`, and `expected_content_hash`; the runtime derives the graph lifecycle state and a caller cannot request the target state. Its result updates route-candidate evidence only and does not populate `claim_verification_refs`, `claim_authorization_refs`, `workflow_authorization`, or `claim_authorizations[]`. After a ready reconciliation, rerun Compass once; final claims still require the separate workflow evidence and authorization below.
