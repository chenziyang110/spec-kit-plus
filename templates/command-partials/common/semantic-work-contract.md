...
`{{specify-subcmd:specify-runtime doctor --format json}}`. If it reports
`bootstrap_required`, stop at that human-owned bootstrap boundary; the agent
must not invoke `uvx`, Python `specify`, or a user-level runtime. After the
human repair, re-open the installed command or reference. If the marker
remains, report cognition unavailable and use live repository evidence only
where this workflow permits degraded advisory navigation.
...
- **Graph reconciliation is not WorkContract authorization**: supply only claim-specific semantic intent to `specify-runtime cognition claim-reconcile prepare`, then execute the returned `apply_argv` through `specify-runtime cognition claim-reconcile apply`. The runtime owns the revision-bound, expiring integrity packet and derives the graph lifecycle state; callers cannot author integrity preconditions or request a target state. Its result updates route-candidate evidence only and does not populate `claim_verification_refs`, `claim_authorization_refs`, `workflow_authorization`, or `claim_authorizations[]`. After a ready reconciliation, rerun Compass once; final claims still require the separate workflow evidence and authorization below.
...
- **Runtime path**: use compass-first navigation for ordinary generated workflows. Run or emulate `specify-runtime cognition compass --intent <intent> --query "$ARGUMENTS" --format json` as the default brownfield intake, then use semantic-intake escalation when the compass packet is draft-like, localized, symptom-first, mixed-language, missing coverage, or needs explicit concept decisions. When that escalation is available, build the WorkContract input in memory and run or emulate `specify-runtime cognition semantic-intake --input-json '<work-contract-input-json>' --format json`; never create a semantic-intake input file. If the command is unavailable, manually produce the same fields from the alias catalog and existing compass/query guidance.
...
- **v1.1 audit artifact**: when semantic-intake escalation influences routing, capture a replayable WorkContract artifact with the semantic-intake input/output snapshot, selected/rejected basis, permission upgrade/downgrade reason, action log, and route corrections. Build only the compact audit input in memory and run `specify-runtime cognition semantic-audit --input-json '<semantic-audit-input-json>' --persist-dir <WORKFLOW_STATE_DIR> --format json`. The cognition CLI atomically materializes the registered `semantic-audit-input.json` and `semantic-audit-output.json`; never recreate, submit, or patch either file. The audit artifact records why the route was chosen; it does not authorize source changes, does not read live source, does not raise permission above P2, and does not replace workflow verification.
- **Audit fallback**: if semantic-audit is unavailable, do not create or replace either canonical audit file. Continue only under the lower available permission, record the blocked/skipped audit status through the workflow-state artifact CLI, and prove any later claims with live evidence.
...
- **v1.3.4 audit state persistence**: persist `semantic-audit-input.json` and `semantic-audit-output.json` next to the active workflow state only with `specify-runtime cognition semantic-audit --persist-dir <WORKFLOW_STATE_DIR>`. Patch `semantic_audit_input_path` and `semantic_audit_output_path` through `specify-runtime artifact patch` with the returned canonical paths; use `<WORKFLOW_STATE_DIR>/semantic-audit-input.json` and `<WORKFLOW_STATE_DIR>/semantic-audit-output.json` when a workflow-state directory exists. On resume, query `semantic_audit_state` and both persisted audit files through targeted `artifact show` calls before recomputing `claim_readiness` or making final claims. If any referenced file is missing, stale, or inconsistent with the current route decision, set `semantic_audit_resume_status` to `missing|stale|needs-rerun` and rerun the audit with `--persist-dir`; never rebuild either file through generic artifact submission or chat memory.
- **v1.3.5 resume validation**: before trusting persisted claim_readiness on resume, compare selected_candidate_ids, active_claim_type, claim_authorization_refs, and claim_verification_refs from workflow state against the `artifact show` result for persisted `semantic-audit-output.json`. Record the comparison through a leased patch to workflow state. If selected candidates, active claim type, authorization refs, verification refs, or route decision fingerprint differ, set `semantic_audit_resume_status: needs-rerun` and rerun `semantic-audit --persist-dir <WORKFLOW_STATE_DIR>` before any final claim.
- **v1.3.6 generated resume smoke**: generated workflows must perform a prompt-level generated resume smoke before trusting persisted semantic-audit state. Verify both audit artifacts with `artifact show`. Compare selected_candidate_ids, active_claim_type, and semantic_audit_route_fingerprint against workflow state plus `semantic-audit-input.json.semantic_audit_input.route_decision`; compare claim_authorization_refs and claim_verification_refs against `semantic-audit-output.json.workflow_authorization` and `semantic-audit-output.json.claim_readiness`. Record `semantic_audit_generated_resume_smoke` and `semantic_audit_stale_reasons` through a leased workflow-state patch. Fingerprint mismatches are route-changed. Use the static semantic audit resume examples at `.specify/templates/examples/semantic-audit-resume/scenarios.md` when available. If any smoke check fails, set `semantic_audit_resume_status: needs-rerun`, keep claim_ready false, and rerun `semantic-audit --persist-dir <WORKFLOW_STATE_DIR>` before any final claim.
- **v1.3.9 optional runtime validator**: when available, generated workflows may build the resume-validation object in memory and run `specify-runtime cognition semantic-audit-resume --input-json '<resume-validation-json>' --format json`. The input contains extracted workflow state plus concrete paths to the CLI-persisted semantic audit artifacts; never create a resume-validation file, and do not require the runtime to parse workflow-state.md. Prompt fallback remains valid when the command is unavailable, blocked, or unnecessary. The validator is a comparison helper only and does not authorize source edits, final claims, or P3/P4 permission. Its output records `validator: semantic-audit-resume`, `can_reuse_persisted_claim_readiness`, `grants_permission: false`, and `boundary: comparison_only_no_source_edit_or_claim_authorization`.
...
- **v1.3.11 resume validator workflow preference**: on resume, prefer the optional runtime validator when a compatible `specify-runtime cognition semantic-audit-resume` command is available. Build the validation object in memory from the current workflow state's semantic_audit_state plus concrete `semantic_audit_input_path` and `semantic_audit_output_path`, run it through `--input-json`, and patch its smoke status, validation status, stale reasons, and reuse decision back into workflow state through the artifact CLI. If the validator returns fresh and `can_reuse_persisted_claim_readiness: true`, the workflow may reuse persisted claim readiness for the same active claim; this still does not grant P3/P4 permission or source edits. If the validator is unavailable, blocked, or returns stale output, prompt fallback remains valid and any failed result keeps final claims blocked until the cognition CLI atomically regenerates the pair through `semantic-audit --persist-dir`.
...
- **v1.3.13 real downstream resume smoke**: generated-project tests verify the validator with workflow-local audit artifacts under an actual downstream state directory, plus an in-memory resume-validation object built from that local state. Treat this as the adoption proof: examples teach the shape, while workflow-local CLI-owned files prove path resolution and resume behavior.
- **Workflow-submitted audit input**: when a workflow uses semantic-intake escalation, carry the `semantic_audit_input` object in memory before broad live reads and submit it once to the cognition CLI with `--persist-dir`; the runtime owns both canonical replay files. The object is named `"semantic_audit_input"` in notes/handoffs and has this minimum shape:
  ```json
  {
