...
- Before substantive execution, present one Understanding Checkpoint using the fixed Quick Checkpoint table card below. It must cover the understood problem, planned outcome, scope boundary, known facts and assumptions, affected surfaces, a concrete ordered implementation plan, next action, validation evidence, and the stop condition that would require escalation; keep the checkpoint plain text for terminal output with no HTML tags or inline line-break markup; wait for user confirmation and record it in quick `STATUS.md`.
