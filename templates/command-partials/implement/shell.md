...
- Primary inputs: `tasks.md`, the plan package, `implement-tracker.md`, worker-result handoff files, passive learning files, the `specify project-cognition query` task-local bundle with readiness and returned `minimal_live_reads`, and the smallest workflow-local state files needed for the touched area.
