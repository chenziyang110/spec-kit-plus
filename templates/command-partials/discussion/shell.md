...
- For pre-handoff readiness, include the likely verdict, proposed handoff goal, recommended consumer, package scope, excluded scope, readiness checks, default next action, and override path without creating a separate assessment artifact.
- Choose the consumer from the user's delivery intent, not task size. Recommend `sp-quick` when the confirmed outcome is implementation-ready and the user wants direct delivery; large, multi-capability, architectural, migration, compatibility, rollout, consequence-heavy, or acceptance-heavy work remains Quick-eligible. Recommend `sp-specify` only when the user explicitly selects a formal spec-first path. Treat an unconfirmed consumer choice as a user-owned handoff decision and include the recommendation in the digest review.
...
- A user reply is not itself a save trigger. A contextual confirmation such as `yes`, `ok`, or `可以` inherits the immediately preceding named decision: it approves a handoff only when the visible request named handoff creation or the exact review digest and consumer choice; otherwise it remains an ordinary decision confirmation and cannot authorize `sp-quick` or `sp-specify` consumption.
...
- If the direction is coherent and boundary-locked after explicit handoff request, submit exactly one draft contract through `specify-runtime discussion write-handoff`; it owns `handoff-to-specify.json`.
