...
- Treat `handoff-ready` as resumable until `sp-specify` consumes it or the user confirms the topic should be dropped; after consumption, mark it with `{{specify-subcmd:specify-runtime discussion mark-consumed <slug> --feature-dir <feature-dir>}}` before archiving.
...
- Before every final response that names `sp-specify`, read the canonical discussion status and withhold the invocation unless it is exactly `handoff-ready`.
