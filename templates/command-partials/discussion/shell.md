...
- If handoff review returns `request-changes` or a downstream consumer reports `blocked_by_handoff_integrity`, repair the handoff in `sp-discussion`: update canonical `handoff-to-specify.json`, render Markdown from the same payload, synchronize protected facts and `source_evidence`, require `version`, `status`, `entry_source: sp-discussion`, `source_handoff`, `source_handoff_json`, `planning_gate_status`, `coverage_status`, `hard_unknown_count`, `open_conflict_count`, `quality_gate`, `review_digest`, and consumer fields, rerun self-review, then ask the user to approve the current digest. Do not make `sp-specify` or `sp-quick` reconstruct or patch the pair.
