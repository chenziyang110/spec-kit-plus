...
- Run `{{specify-subcmd:discussion validate-handoff <slug> --mode draft --json}}` after writing and self-reviewing the draft. Present its exact digest for named user confirmation, then record that approval with `{{specify-subcmd:discussion confirm-handoff <slug> --digest <review-digest> --json}}` before `{{specify-subcmd:discussion mark-ready <slug> --json}}`.
