...
Deterministic field/schema/digest failures are repaired through `{{specify-subcmd:specify-runtime discussion validate-handoff}}` and `sp-discussion`. Review judgment must not silently patch upstream truth.
