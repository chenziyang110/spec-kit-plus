...
- Use map-update for ordinary existing-baseline gaps. Use map-scan -> map-build
  ...
  Schema v4 is current-only. The runtime does not migrate schema v3 or older
...
- Default project cognition intake is `project-cognition compass --intent <intent> --query="$ARGUMENTS" --format json`.
  ...
  For decisive claim-specific evidence, create a current-only `project-cognition claim-reconcile --input <packet.json> --format json` packet with `claim_reconciliation_contract_version: 1`, `expected_generation_id`, workflow, RFC3339 `observed_at`, stable `claim_id`, `expected_state`, reason, and bounded evidence containing `source_path`, `span`, role, and `expected_content_hash`. The runtime re-reads non-ignored paths and the runtime derives the state; never send a target state such as `requested_state`. Generic workflow verification is insufficient. On ready, rerun Compass once; on partial or blocked, withhold the claim.
