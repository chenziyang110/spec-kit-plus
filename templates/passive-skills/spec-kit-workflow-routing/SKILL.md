...
- Use `sp-quick` for non-trivial direct delivery that needs resumable state,
  task-local planning, lanes or batches, and verification. It is not limited by
  task size, capability count, architecture, migration, compatibility, rollout,
  consequence breadth, or acceptance depth.
- `sp-quick` performs one Understanding Checkpoint before substantive execution for a raw quick request. A user-confirmed discussion contract with no quick-stage `semantic_delta` reuses its confirmed digest instead of asking again. When a checkpoint is needed:
  ...
  request/outcome, visible result, scope, ordered work items and dependencies,
  work-item acceptance, recommended approach, assumptions/risks, overall
  completion evidence, and reconfirmation trigger. Use stable Q1/Q2 ids for one
  or many deliverables. Confirm deliverable-level order while keeping internal
...
- A valid explicit handoff from discussion is one canonical agent-only JSON contract: `handoff-to-specify.json`, with self-review and user confirmation. Before drafting, present both paths and their eligibility, explain any blocker, and give a complexity-informed recommendation among eligible paths: recommend `sp-quick` for bounded, well-understood direct delivery with stable requirements and limited cross-boundary risk; recommend `sp-specify` for high-complexity or ambiguous work, interacting capabilities or systems, architecture/data migration/security/compliance/rollout concerns, broad acceptance obligations, or durable specification traceability. Task size alone is not a hard routing ceiling. The user's final choice controls the route, including when it differs from the recommendation; record it as `recommended_consumer` only after the user chooses an eligible path. Pass the JSON path or discussion slug to that selected consumer. Require the matching `consumer_eligibility.<consumer>.status: ready`; when exactly one unconsumed `handoff-ready` discussion exists, that selected consumer may consume it directly. Before the ready contract exists, do not tell the user to run either downstream consumer; the next action is `sp-discussion` handoff creation, review, or repair. `specification-input.md` is not a substitute handoff. Both consumers validate the user-confirmed quality gate, zero hard unknowns, zero open conflicts, and source-contract integrity; `sp-specify` additionally compiles the formal feature contract before feature creation, while `sp-quick` binds the source contract and digest into its task-local `STATUS.md`.
...
- Use `sp-implement` after `sp-tasks` produces canonical `task-index.json` in any execution mode, renders `tasks.md`, and records `/sp.implement`. Light mode keeps this package compact; it does not bypass the CLI-owned index. `sp-implement` selects leader-direct or delegated execution per task, compiles delegated packets just in time, runs event-triggered review, and records result/validation/review/recovery once in the task lifecycle record. Product goal, scope, architecture, required evidence, `MP-*`, `CA-###`, and feasibility conflicts route back to their upstream owner.
...
- Use `sp-review` after successful implementation closeout. It must start the integrated product from official real entrypoints, exercise required user journeys and interaction/consumer wiring, record findings, and repair every approved-scope defect. The Leader builds the Review Universe, dispatches independent read-only Audit subagents, joins every packet, dispatches isolated Fix subagents, and assigns independent Revalidation subagents; only the Leader may declare zero uncovered coverage or the final verdict. The failed/dependent/affected set scopes finding-level revalidation, but after any Fix the Leader reruns every required Review scenario and recaptures all required evidence against the final snapshot. Review alone creates the immutable `reviewed_runtime_targets` and target digest used by Acceptance. Unknown mechanisms stay inside Review through read-only diagnostic packets, and repair size or missing code never sends work back to implementation/tasks. Only proven requirement, design, or architecture truth gaps reopen an upstream workflow.
- Use `sp-accept` only after fresh approved `sp-review` closeout. It assumes the human remembers nothing, restores product context, guides one real-entrypoint step at a time against Review's immutable target, persists each actual reply as a structured human confirmation bound to its confirmation id, target, and reviewed snapshot, and never edits source. Every failed product/runtime observation returns to Review for diagnosis, an independent Fix, and independent revalidation in a new Review cycle; only Review may route a proven truth gap upstream. Any repair resets every frozen Human Acceptance scenario and invalidates every prior human PASS and confirmation, so Acceptance reruns the full frozen Human Acceptance Universe after Review approval, using the prior failed cursor only as the first retest point. Technical checks and silence never count as human PASS.
...
Recommend against `fast` when a request triggers the Senior Consequence Analysis Gate. Use `quick` for direct-delivery consequence work of any breadth, with durable `STATUS.md` state and expanded task-local `PLAN.md` material when needed. Resolve lifecycle semantics, running work, destructive policy, shared state, downstream consumers, and acceptance decisions through the Quick Checkpoint Amendment contract. Recommend `discussion` when product direction is not ready for implementation, and recommend `specify` only when the user explicitly wants a formal spec-first workflow. Recommend `debug` when the issue is a failure with unknown root cause.
...
- Tracked direct delivery without a formal spec first: `{{invoke:quick}}`
