...
- **Technical Source of Truth**: Maintain
  ...
  localized stale cognition runtime refresh and ordinary changed-path
  maintenance; use `map-scan` followed by `map-build` only when the baseline is
  missing, unusable, schema-incompatible, explicitly being rebuilt, or
  invalidated by broad architecture replacement, or when coverage gaps are
  unadoptable after update classification.
