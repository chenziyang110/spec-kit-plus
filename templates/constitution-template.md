...
- **Technical Evidence**: Treat `.specify/project-cognition/status.json` and
  ...
  Recommend `map-update` for localized stale cognition or ordinary changed-path
  map maintenance; recommend `map-scan` followed by `map-build` only when the
  user wants repair for a missing, unusable, schema-incompatible, explicitly
  rebuilt, or architecture-replaced baseline.
