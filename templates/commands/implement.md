...
- When execution changes map-level truth surfaces, refresh the project cognition runtime through `/sp-map-update` when the touched area is localized, then use `{{specify-subcmd:project-cognition complete-refresh --format json}}` as the successful-refresh finalizer. Rebuild through `/sp-map-scan` followed by `/sp-map-build` only when no usable localized baseline remains or a full rebuild is required.
...
3. Load and analyze the implementation context:
   ...
   - **REQUIRED**: Query project cognition with `{{specify-subcmd:project-cognition query --intent implement --query "$ARGUMENTS" --format json}}`.
...
10. Completion validation:
   ...
   - If the completed implementation changed truth-owning surfaces, shared surfaces, command/route/contract boundaries, verification entry points, runtime assumptions, or other map-level coverage facts, and verification is truthfully green and no explicit blocker prevents completion, including unresolved `open_gaps`, refresh the project cognition runtime through `{{invoke:map-update}}` when the touched area is localized; rebuild through `{{invoke:map-scan}}`, then `{{invoke:map-build}}` only when no usable localized baseline remains or a full rebuild is required; then run `{{specify-subcmd:project-cognition complete-refresh --format json}}` as the successful-refresh finalizer.
