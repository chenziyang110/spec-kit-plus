...
3. Load and analyze the implementation context:
   ...
   - **IF READINESS IS `needs_rebuild`**: Treat map output as advisory, continue with live repository evidence, and recommend `/sp-map-scan` followed by `/sp-map-build` as follow-up map maintenance when the user wants map repair.
   - **IF READINESS IS `needs_update` OR TOO WEAK FOR THE TOUCHED AREA**: Treat map output as advisory, continue with live repository evidence, and recommend `/sp-map-update` with the changed paths as follow-up map maintenance.
   ...
   - **IF TASK-RELEVANT COVERAGE IS INSUFFICIENT**: use returned testing artifacts when helpful, inspect the returned minimum live files needed to replace guesswork with evidence, and recommend `/sp-map-update` with changed paths or affected surfaces as follow-up map maintenance; recommend `/sp-map-scan` followed by `/sp-map-build` only when the baseline is missing, unusable, schema-incompatible, explicitly being rebuilt, or invalidated by broad architecture replacement.
