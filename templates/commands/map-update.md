...
- Before `validate-build` or `complete-refresh`, run the single planner-first update path:

```text
{{specify-subcmd:specify-runtime cognition closeout-plan --workflow sp-map-update --intent implement --reason map-update --changed-path "<accepted-path>" --format json}}
```

Pass every accepted path as its own explicit `--changed-path`; do not let unrelated dirty or untracked paths expand the boundary. Complete only the returned agent-owned evidence fields and unknown-path dispositions, then execute structured `update_argv`. Every `unknown_path_dispositions[].agent_disposition` is bound to the matching `path_changes[].disposition`; missing, duplicate, conflicting, or unmatched decisions fail before graph mutation, and delta mode must supply every returned `--path-disposition` placeholder. An ignored disposition remains in audit-only `path_changes`, but it must not enter graph-changing `changed_paths` or create graph adoption/reconciliation records. Legacy or free-text verification is audit evidence with `result=recorded` only and cannot satisfy clean closeout; clean closeout requires a structured verification object with exact `result=passed`. Display-only command strings are never executable instructions, and this workflow must not construct a second direct delta or payload update path.

Use the returned `result_state` as the completion gate, not `status=ok` alone. For `result_state=ready` or `result_state=no_op`, run `{{specify-subcmd:specify-runtime cognition validate-build --format json}}` after the latest update. Only `status=ok` with `readiness=query_ready` creates the validate-build receipt bound to the latest update ID, outcome, and active generation; then, and only then, run `{{specify-subcmd:specify-runtime cognition complete-refresh --format json}}`. Clean completion requires that receipt-bound finalizer to succeed; until it does, the runtime gate withholds Compass/query as pending finalization. `partial_refresh` must preserve review data and must not run `complete-refresh`; `needs_rebuild` must route to `{{invoke:map-scan}}`, then `{{invoke:map-build}}`; `blocked` must report the blocker and recovery condition; legacy `recorded` is never clean completion.

- After applying update records, apply the receipt-bound validation and finalizer gate above exactly once.
- `sp-map-update` must not call `complete-refresh` when `result_state` is `partial_refresh`, `needs_rebuild`, `blocked`, or legacy `recorded`; those states are useful recorded outcomes, not fresh completed baselines.
...
- If the re-evaluated runtime has `status=ok` with `readiness=query_ready`, finalize the successful refresh through the receipt-bound `{{specify-subcmd:specify-runtime cognition complete-refresh --format json}}` path so cognition freshness metadata cannot remain stale.
...
- After those source changes are committed, a matching validation receipt may be consumed by `{{specify-subcmd:specify-runtime cognition record-refresh --reason "map-update" --format json}}` only as a compatibility or explicit metadata-recording entrypoint. It is not an ordinary workflow-closeout branch and must never bypass the `ready`/`no_op` receipt-bound `complete-refresh` path. Record that metadata without rerunning `{{invoke:map-scan}}` or `{{invoke:map-build}}` unless validation reports `needs_rebuild`, the baseline is unusable, or the affected closure cannot be bounded safely.
...
- when the post-recording freshness result is ready, a completed cognition refresh finalizer via `{{specify-subcmd:specify-runtime cognition complete-refresh --format json}}`
...
- reconcile an affected claim only from claim-specific bounded evidence through `specify-runtime cognition claim-reconcile prepare`, then execute its returned `apply_argv` through `specify-runtime cognition claim-reconcile apply`; generic verification and `result_state=ready` must not re-promote claims
