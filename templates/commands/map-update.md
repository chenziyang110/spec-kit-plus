...
```text
{{specify-subcmd:project-cognition closeout-plan --workflow sp-map-update --intent implement --reason map-update --changed-path "<accepted-path>" --format json}}
```
...
- If the re-evaluated runtime has `status=ok` with `readiness=query_ready`, finalize the successful refresh through the receipt-bound `{{specify-subcmd:project-cognition complete-refresh --format json}}` path so cognition freshness metadata cannot remain stale.
...
- After those source changes are committed, a matching validation receipt may be consumed by `{{specify-subcmd:project-cognition record-refresh --reason "map-update" --format json}}` only as a compatibility or explicit metadata-recording entrypoint. It is not an ordinary workflow-closeout branch and must never bypass the `ready`/`no_op` receipt-bound `complete-refresh` path. Record that metadata without rerunning `{{invoke:map-scan}}` or `{{invoke:map-build}}` unless validation reports `needs_rebuild`, the baseline is unusable, or the affected closure cannot be bounded safely.
