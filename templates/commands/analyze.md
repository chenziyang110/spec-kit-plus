...
- If readiness is `needs_rebuild`, treat map output as advisory, continue with live repository evidence, and recommend `{{invoke:map-scan}}`, then `{{invoke:map-build}}` as follow-up map maintenance when the user wants map repair.
- If readiness is `needs_update` or the returned bundle is too weak for the touched area, treat map output as advisory, continue with live repository evidence, and recommend `{{invoke:map-update}}` with the changed paths or affected surfaces as follow-up map maintenance.
- Recommend `{{invoke:map-scan}}`, then `{{invoke:map-build}}` only when the baseline is missing, unusable, schema-incompatible, explicitly requested for rebuild, or invalidated by broad architecture replacement.
