...
3. Preserve complete-first scope and map every `CA-###`, `MP-*`, preserved create/scaffold or other capability operation, reference-fidelity item, and user-observable UI/TUI/CLI/API/runtime path before finalizing `tasks.md`. Every UI-bearing task—not only screenshot-driven work—materializes the exact v2 object referenced by `.specify/templates/task-index-template.json#/ui_contract_schema_ref` from `.specify/templates/task-packet-template.json#/ui_contract`, then renders v2 Design Quality Coverage plus compact per-task `Scope Boundaries` and `UI Implementation Contract` projections: work/surface/platform types, subject/audience/job, three theses, signature, approved visual ref, task-relevant reference/content/image records, `ui_fidelity_mode`, states, must-preserve/adapt/not rules, `required_evidence`, difference inventory, accepted deviations, and real-entrypoint proof under `real_entrypoint_evidence`. Required evidence includes `structure_snapshot`, `visual_capture`, `runtime_diagnostics`, and comparison/human review; synthetic component proof is insufficient.
