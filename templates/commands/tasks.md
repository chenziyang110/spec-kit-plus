...
5. Use task-index.json as the canonical task graph for standard/heavy and every UI-bearing task set, then render `tasks.md`; only light non-UI leader-direct work may remain a compact `tasks.md`. Record `official_entrypoints` plus a minimal complete `system_review_scenarios` matrix covering startup, each changed user-observable journey, and affected shared regression paths so `sp-review` does not reconstruct executable acceptance from prose. Do not pre-generate a full worker packet for every task. Record enough task shape for `sp-implement` to compile delegated packets just in time from the current repository state.
