...
3. Preserve complete-first scope and map every `CA-###`, `MP-*`, preserved create/scaffold or other capability operation, reference-fidelity item, and user-observable UI/TUI/CLI/API/runtime path before finalizing `tasks.md`. Every UI-bearing task—not only screenshot-driven work—carries Design Quality Coverage plus exact per-task `Scope Boundaries` and `UI Implementation Contract` sections with `ui_fidelity_mode`, design inputs, required states, `required_evidence`, difference inventory, accepted deviations, and real-entrypoint proof under `real_entrypoint_evidence`.
