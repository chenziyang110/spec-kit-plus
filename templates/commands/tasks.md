...
- Do not hand off directly to `{{invoke:implement}}` from `sp-tasks`; the clean package should hand off to implement unless explicit legacy analyze state is recorded.
...
4. **Execute task generation workflow**:
   ...
   - Update `task-generation/evidence-index.json` after each accepted lane handoff with lane id, handoff path, source artifacts inspected, decisions or constraints contributed, affected task IDs or batch IDs, blocker status, and integration status.
    ...
    - **Implementation-Readiness Task Self-Audit**: Before finalizing `tasks.md`, run the implementation-readiness checks against the generated task package.
