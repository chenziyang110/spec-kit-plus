...
- Do not create the JSON handoff before explicit request plus a locked Context Boundary Gate, or invoke/recommend `sp-quick` or `sp-specify` before self-review and user confirmation of the protected revision and consumer choice.
...
7. When ready for review, choose `recommended_consumer` from the confirmed delivery intent, never from a task-size ceiling: `sp-quick` for direct delivery of any size, `sp-specify` only for an explicitly selected formal spec-first path. Build only the semantic input—such as `consumer_eligibility`, `recommended_consumer`, `planning_constraints`, and `discussion_decision_digest`—record agent self-review, then submit it with `{{specify-subcmd:specify-runtime discussion write-handoff <slug> --input-json '<semantic-json>' --json}}` followed by `{{specify-subcmd:specify-runtime discussion validate-handoff <slug> --mode draft --json}}`. The CLI expands stable defaults, metadata, and schema structure. Never read the template or create a draft JSON file; the discussion runtime alone materializes the canonical handoff.
...
10. Before every final response that names `sp-quick` or `sp-specify`, run `{{specify-subcmd:specify-runtime discussion status <slug> --format json}}` and consume the returned status and canonical `recommended_consumer`. Mention only that selected downstream invocation, and only when status is `handoff-ready`; otherwise keep the next action inside `sp-discussion` assessment, draft review, or repair.
