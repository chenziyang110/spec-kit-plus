...
1. Run `{SCRIPT}` from repo root once (`--json --paths-only` / `-Json -PathsOnly`) and parse the available feature paths.
   - If `FEATURE_DIR` is not already explicit, prefer `{{specify-subcmd:specify-runtime lane resolve --command explain --ensure-worktree}}` before guessing from branch-only context.
