...
- [AGENT] If task-relevant cognition coverage is insufficient for the failing area, stop and tell the user to refresh through `{{invoke:map-update}}` with changed paths or affected surfaces; rebuild through `{{invoke:map-scan}}`, then `{{invoke:map-build}}` only when the baseline is missing, unusable, schema-incompatible, explicitly being rebuilt, invalidated by broad architecture replacement, or blocked by unadoptable coverage gaps; wait for that refresh before root-cause analysis continues.
