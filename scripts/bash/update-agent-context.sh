...
- Prefer native subagents for safe packetized lanes before considering managed-team or leader-inline fallback.
...
`spec-lint` is a zero-dependency binary that mechanically validates spec artifact sets before they enter planning. It runs 8 checks tiered by task risk:
