...
- The project cognition runtime is query-backed: use `specify project-cognition query` to retrieve the task-local project cognition bundle before broader repository analysis.
- Treat raw graph JSON artifacts as obsolete runtime surfaces; use returned readiness and `minimal_live_reads` instead of direct graph or slice reads.
...
- Supporting handbook/project-map artifacts are support-only or reference-only compatibility/export surfaces for ordinary workflow execution; do not treat them as the primary runtime truth path.
- Before planning, debugging, or implementing against existing code, query the workflow-appropriate project cognition bundle required by that workflow.
- If the cognition baseline is missing, run `sp-map-scan` followed by `sp-map-build` before continuing.
- Treat project cognition query readiness as the truth source.
...
- Do not treat consumed project cognition query context as self-maintaining.
...
- `specify project-cognition query` determines whether query-backed cognition coverage can be trusted as fresh and records the git-aligned baseline freshness truth source.
