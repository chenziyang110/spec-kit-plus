...
func TestPrepareDerivesSnapshotHashesTimestampAndCanonicalPacket(t *testing.T) {
	...
	if prepared.PreparedPacketPath == "" || len(prepared.ApplyArgv) != 8 {
		t.Fatalf("prepared = %#v, want runtime packet path and structured apply argv", prepared)
	}
	if strings.Join(prepared.ApplyArgv, " ") != "specify-runtime cognition claim-reconcile apply --input "+prepared.PreparedPacketPath+" --format json" {
