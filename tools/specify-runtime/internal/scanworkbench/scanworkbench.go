...
func renderPacket(packetID string, paths []string) string {
	...
	builder.WriteString("\n```\n\n")
	builder.WriteString("Treat this as a cumulative checkpoint skeleton. Move only concretely completed paths from ledger.todo to ledger.done, add matching paths_read, coverage, evidence, and nodes[].paths, increment sequence, and keep acceptance=partial even when every assigned path is complete; scan-accept derives pass only after runtime validation. Submit through specify-runtime cognition scan-checkpoint --result-json (inline, @path, or - for stdin). Prefer @path on Windows when the JSON is large. Never create or edit the runtime-owned result path. Finish with scan-accept or yield remaining work with scan-yield.\n")
	builder.WriteString("Edge endpoint rules (enforced at validate-scan and build): prefer unique node ids for source/target. A concrete repository path is allowed only when exactly one provisional node owns that path after merge. If a shared contract/skill file is listed on multiple nodes' paths, do not use that path as an edge endpoint—point at one explicit aggregate node id. Every node must set confidence (for example medium, high, or verified).\n")
	builder.WriteString("Do not read an unassigned target merely to strengthen the edge. Do not write product files or canonical cognition ledgers.\n")
