...
func normalizePlanJSON(data []byte) ([]byte, PlanDiagnostics, error) {
	...
	if generationID, ok := payload["lexicon_generation_id"].(string); !ok || strings.TrimSpace(generationID) == "" {
		...
		appendDiagnostic(&diagnostics.RepairHints, "Carry lexicon_generation_id from the specify-runtime cognition lexicon payload into the query_plan.")
	}
	...
	if !ok || version != float64(CandidateUniverseVersion) {
		return nil, diagnostics, planParseError(
			...
			diagnostics.Warnings,
			append(diagnostics.RepairHints, "Carry candidate_universe_version from the current specify-runtime cognition lexicon payload into the query_plan."),
		)
	}
