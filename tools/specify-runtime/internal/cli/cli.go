...
import (
	...
	"github.com/chenziyang110/spec-kit-plus/tools/specify-runtime/internal/changes"
	changemodel "github.com/chenziyang110/spec-kit-plus/tools/specify-runtime/internal/changes/model"
	"github.com/chenziyang110/spec-kit-plus/tools/specify-runtime/internal/closeout"
	...
	"github.com/chenziyang110/spec-kit-plus/tools/specify-runtime/internal/reconcile"
	"github.com/chenziyang110/spec-kit-plus/tools/specify-runtime/internal/reference"
	...
	"github.com/chenziyang110/spec-kit-plus/tools/specify-runtime/internal/scanworkbench"
	"github.com/chenziyang110/spec-kit-plus/tools/specify-runtime/internal/store"
	"github.com/chenziyang110/spec-kit-plus/tools/specify-runtime/internal/update"
...
func Run(args []string, stdout io.Writer, stderr io.Writer, version string) int {
	...
	if args[0] == "--version" {
		fmt.Fprintf(stdout, "specify-runtime cognition %s\n", version)
		return 0
	}
	if args[0] == "version" {
...
func claimReconcileCommand(args []string, stdout io.Writer, stderr io.Writer, paths rt.Paths) int {
	if len(args) == 0 {
		fmt.Fprintln(stderr, "usage: specify-runtime cognition claim-reconcile prepare|apply [options]")
		return 2
	}
	switch args[0] {
	case "prepare":
...
func addStatusAccessRecovery(payload map[string]any, paths rt.Paths, causeCode runtimegate.CauseCode, repairErr error) {
	...
	resumeArgv := []string{"specify-runtime", "cognition", "repair-status", "--format", "json"}
	verifyArgv := []string{"specify-runtime", "cognition", "status", "--format", "json"}
	...
	payload["human_action"] = map[string]any{
		...
		"prerequisites": []string{
			"Run these steps from the repository root with the same user account that runs the project-pinned specify-runtime binary.",
			"Stop other specify-runtime processes or editors that may be holding the status file open.",
		...
		"steps": []map[string]any{
			...
			{
				"step":            4,
				"instruction":     "Run the exact resume argv through the project-pinned specify-runtime launcher, then retain its complete JSON result.",
...
func validateBuildCommand(args []string, stdout io.Writer, stderr io.Writer, paths rt.Paths) int {
	...
	if receiptErr != nil {
		fmt.Fprintf(stderr, "specify-runtime cognition: record validate-build receipt: %v\n", receiptErr)
		return 1
	}
	if payloadBlocked(payload) {
		return 1
...
func deltaCommand(args []string, stdout io.Writer, stderr io.Writer, paths rt.Paths) int {
	if len(args) == 0 {
		fmt.Fprintln(stderr, "specify-runtime cognition: delta subcommand is required")
		return 2
	}
	switch args[0] {
	case "begin":
	...
	default:
		fmt.Fprintf(stderr, "specify-runtime cognition: unknown delta subcommand: %s\n", args[0])
		return 2
	}
}
...
func writeJSON(w io.Writer, payload any) int {
	...
	if err := encoder.Encode(payload); err != nil {
		fmt.Fprintf(os.Stderr, "specify-runtime cognition: encode json: %v\n", err)
		return 1
	}
	return 0
}
