...
func TestVersionPrintsBinaryName(t *testing.T) {
	...
	if strings.TrimSpace(stdout.String()) != "specify-runtime cognition v1.2.3" {
...
func TestRepairStatusCommandReturnsOperatorTutorialWhenStatusPathCannotBeWritten(t *testing.T) {
	...
	if !reflect.DeepEqual(jsonAnySliceStrings(verification["argv"].([]any)), []string{"specify-runtime", "cognition", "status", "--format", "json"}) {
		t.Fatalf("independent_verification = %#v", verification)
	}
	resume := human["exact_resume"].(map[string]any)
	if !reflect.DeepEqual(jsonAnySliceStrings(resume["argv"].([]any)), []string{"specify-runtime", "cognition", "repair-status", "--format", "json"}) {
...
func TestCompassActiveGenerationMismatchReturnsExecutableStatusRepair(t *testing.T) {
	...
	action := payload["recommended_next_action"].(map[string]any)
	if !reflect.DeepEqual(jsonAnySliceStrings(action["argv"].([]any)), []string{"specify-runtime", "cognition", "repair-status", "--format", "json"}) {
