...
- **Debug workflow confirmation**: `sp-debug` also performs one Debug Understanding Checkpoint before substantive investigation. Generated agents must confirm the symptom, expected behavior, investigation boundary, first evidence approach, and success signal before reproduction, logs, source/test reads, evidence collection, fixes, or validation.
