...
Schema v4 adds an auditable live feedback loop and a current evidence basis while preserving superseded claim evidence as history. Decisive claim-specific bounded reads use a current-only `claim_reconciliation_contract_version=1` packet and `project-cognition claim-reconcile --input <packet.json> --format json`; the runtime verifies active generation/state plus file hashes, derives the graph lifecycle state, and returns `rerun_compass_once` when ready. This remains route-candidate evidence and cannot authorize a workflow final claim. Schema v4 does not migrate schema v3 or older databases.
...
- **Agent-native phase pipeline**: `sp-discussion` emits one confirmed Agent-only JSON requirement contract; `sp-specify` compiles it into `spec-contract.json` and asks again only for a non-empty user-owned `semantic_delta`; `sp-plan` writes `plan-contract.json` plus only triggered research/data/API/quickstart artifacts; `sp-tasks` writes `task-index.json` for standard/heavy work and leaves WorkerTaskPacket generation to `sp-implement` just in time; `sp-implement` keeps one lifecycle record per task and reviews on drift, joins, validation failure, obligation conflict, or review-window limits instead of after every batch. Human-facing project documents and replies remain human-first; handoffs are optimized only for Agent consumption.
...
- **Quick workflow confirmation**: Raw quick requests use one Understanding Checkpoint. A confirmed discussion contract with no quick-stage semantic delta reuses its `review_digest` and does not repeat that confirmation.
...
- After a successful `sp-map-update`, committing the refreshed source changes does not require a full rebuild by itself; update git-baseline freshness with `project-cognition record-refresh` or `project-cognition complete-refresh` unless validation reports `needs_rebuild`.
