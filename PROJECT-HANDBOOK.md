...
- Use live code reads whenever they are needed to prove claims; map coverage can point to likely files, owners, and verification routes.
- If changed paths are missing from project cognition `path_index`, recommend `sp-map-scan -> sp-map-build` as follow-up map maintenance; repeating `sp-map-update` cannot create absent path coverage.
