...
- **Workflow contract generation**: `templates/commands/`, `templates/command-partials/`, and `templates/passive-skills/` define `sp-*` behavior for downstream agents. `sp-specify` is now a collaborative reviewed specification flow: explore project context, ask one question at a time, decompose semantic terms, compare approaches, write artifacts, self-review them, and ask for user review before routing to `/sp.plan`, `/sp.clarify`, or `/sp.deep-research`. Discussion-originated specs must read discussion source files and record capability-like upstream signals in `source_signal_disposition` instead of trusting only the handoff summary. Read `templates/commands/**`, `templates/command-partials/**`, and `modules/templates-generated-surfaces/WORKFLOWS.md`.
