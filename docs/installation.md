...
Discussion sessions remain visible to resume while their status is `active`, `blocked`, or `handoff-ready`. After a handoff has been consumed or the user abandons the topic, use `specify discussion close <slug> --status completed|abandoned` and then `specify discussion archive <slug>` to move it under `.specify/discussions/archive/`.
