...
- Use `map-update` for localized stale cognition refresh recommendations, ordinary changed-path map maintenance, and ordinary existing-baseline gaps; recommend `map-scan` followed by `map-build` only for missing or unusable baseline, schema failure, zero active-generation `path_index` rows, `explicit_rebuild_requested`, or `baseline_identity_invalid`.
