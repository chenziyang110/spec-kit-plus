...
If you're starting from an existing codebase, consult `.specify/project-cognition/status.json` plus the workflow-appropriate slice when available. For the first brownfield cognition baseline, run `sp-map-scan` followed by `sp-map-build` when you want map maintenance and require `project-cognition validate-scan --format json` plus `project-cognition validate-build --format json` to pass for that map-maintenance workflow. Downstream workflows use cognition freshness in `.specify/project-cognition/status.json` as map-quality diagnostics; source-changing `sp-*` workflows run inline project cognition update for their own closeout, while `map-update` is external/manual localized stale cognition maintenance and follow-up repair, with uncertain closure recorded as partial/low-confidence facts, known unknowns, and `minimal_live_reads`.
