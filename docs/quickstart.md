...
> [!TIP]
> **Run Isolation**: Launch-capable integrations submit every `sp-*`/`spx-*` invocation as an independent Run, including the first, through the single-call `specify-runtime run launch ... -- <agent argv>` adapter boundary. The runtime allocates a dedicated Git worktree, forces the child agent cwd, and binds feature resolution to the Run subject instead of a branch-name guess; `run supervise` restarts a previously queued or interrupted Run. `sp-auto` routes only the current managed Run subject; an unbound read-only invocation stops when more than one safe candidate exists.
...
Required phase order is separate in CLI-owned `FEATURE_DIR/workflow.json`.
When the active stage's artifacts are ready, its owner runs
`specify-runtime workflow complete-stage`; the destination stage then executes the exact
returned `specify-runtime workflow transition --to <stage>` argv. Only completed
...
Use evidence-backed `specify-runtime workflow reopen` for an invalidated earlier or same
completed non-accept stage. Blockers are single-owner records: after the stated
criteria are proven, `specify-runtime workflow resolve` reactivates that owner while
...
- During explicit map-maintenance or repair work, invoke the runtime launcher from `.specify/config.json` with `specify-runtime cognition validate-build --format json`, then `specify-runtime cognition complete-refresh --format json` only when build acceptance passes.
- Ordinary workflows should report changed paths and run planner-first project cognition update for their own source-changing closeout. Recommend `map-update` only for external/manual localized stale cognition maintenance, ordinary existing-baseline gaps, and weak localized coverage after a usable baseline; completion still depends on live code, tests, scripts, configuration, or authoritative docs. Use `map-scan -> map-build` only for first/missing/unusable baseline, schema failure, zero active-generation `path_index` rows, `explicit_rebuild_requested`, or `baseline_identity_invalid`.
...
- `discussion` to shape a rough idea before formal specification or direct Quick delivery. At handoff it presents both paths and their eligibility, explains any blocker, and makes a complexity-informed recommendation among eligible paths: Quick for bounded, well-understood work with stable requirements and limited cross-boundary risk; Specify for high ambiguity/complexity, interacting capabilities or systems, architecture/data migration/security/compliance/rollout concerns, broad acceptance obligations, or durable specification traceability. Task size alone is not a hard routing ceiling. The user makes the final choice; only then does `recommended_consumer` record that selection in the single machine-oriented `handoff-to-specify.json`. When the contract introduces no quick-stage `semantic_delta`, Quick binds understanding to the confirmed `review_digest` and does not ask for the same confirmation again.
...
- Use project cognition commands for freshness:
  ...
  - Command shape: `specify-runtime cognition complete-refresh`
  - Command shape: `specify-runtime cognition mark-dirty --reason "<reason>"`
