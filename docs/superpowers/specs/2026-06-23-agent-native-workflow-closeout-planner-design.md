...
-> project-cognition update --payload-file <payload> --reason workflow-finalize --format json
...
- `unknown_paths` should be copied into `payload_draft.known_unknowns` only as explicit uncertainty, not as proven graph coverage.
...
3. If mutation occurred, call:
   ...
   ```text
   project-cognition closeout-plan --workflow <active-workflow> --format json
   ```

4. Consume `payload_draft`, `required_agent_fields`, `ignored_paths`, `unknown_paths`, `recommended_next_command`, and `finalizer_policy`.
5. Fill missing agent-owned fields with live evidence from the completed workflow:
   ...
   - `user_decisions`
   - `boundary`
6. Write the payload file at the planner's `payload_path`.
7. Run the planner's `update_command`.
8. Gate completion on `result_state`, not on `status=ok`, `update_id`, `last_update_id`, or freshness alone.
9. Use `mark-dirty` only when planner/update cannot record useful update data, cannot identify workflow-owned scope, or workflow verification is not trustworthy.
...
After the agent writes the payload and runs update:
...
- Unknown paths appear as uncertainty, not as proven coverage.
...
3. Add Python runtime compatibility requirement for `closeout-plan`.
