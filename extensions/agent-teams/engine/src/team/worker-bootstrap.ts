...
import { codexHome, listInstalledSkillDirectories, specifyRuntimeStateDir } from "../utils/paths.js";
...
const AGENTS_LOCK_PATH = [".specify", "runtime", "state", "agents-md.lock"];
...
function buildWorkerRootAgentsBackupPath(
  ...
  const repoRoot = tryReadGitValue(worktreePath, [
    "rev-parse",
    "--show-toplevel",
  ]);
  const gitPath = repoRoot && resolve(repoRoot) === resolve(worktreePath)
    ? tryReadGitValue(worktreePath, [
        "rev-parse",
        "--git-path",
...
This file is generated for a live Specify team worker run and is disposable.
...
3. Send startup ACK before task work:
   ...
   \`sp-teams api send-message --input "{\"team_name\":\"${options.teamName}\",\"from_worker\":\"${options.workerName}\",\"to_worker\":\"leader-fixed\",\"body\":\"ACK: ${options.workerName} initialized\"}" --json\`

4. Resolve canonical team state root in this order: \`SPECIFY_TEAM_STATE_ROOT\` env -> worker identity \`team_state_root\` -> config/manifest \`team_state_root\` -> local cwd fallback.
...
6. Use claim-safe lifecycle APIs only:
   - \`sp-teams api claim-task --json\`
   - \`sp-teams api transition-task-status --json\`
   - \`sp-teams api release-task-claim --json\` only for rollback to pending
7. Use mailbox delivery flow:
   - \`sp-teams api mailbox-list --input "{\"team_name\":\"${options.teamName}\",\"worker\":\"${options.workerName}\"}" --json\`
   - \`sp-teams api mailbox-mark-delivered --input "{\"team_name\":\"${options.teamName}\",\"worker\":\"${options.workerName}\",\"message_id\":\"<MESSAGE_ID>\"}" --json\`
8. Preserve leader steering via inbox/mailbox nudges; task payload stays in inbox/task JSON, not this file.
9. Do not pass \`workingDirectory\` to legacy team_* MCP tools; use \`sp-teams api\` CLI interop.
...
3. Send an ACK to the lead using CLI interop \`sp-teams api send-message --json\` (to_worker="leader-fixed") once initialized
4. Resolve canonical team state root in this order:
   - SPECIFY_TEAM_STATE_ROOT env
   - worker identity team_state_root
   - team config/manifest team_state_root
   - local cwd fallback (.specify/runtime/state)
...
7. Request a claim via CLI interop (\`sp-teams api claim-task --json\`); do not directly set lifecycle fields in the task file
...
10. On completion/failure, use lifecycle transition APIs:
   - \`sp-teams api transition-task-status --json\` with from \`"in_progress"\` to \`"completed"\` or \`"failed"\`
   - Include \`result\` (for completed) or \`error\` (for failed) in the transition patch
11. Use \`sp-teams api release-task-claim --json\` only for rollback/requeue to \`pending\` (not for completion)
...
15. For legacy team_* MCP tools (hard-deprecated), switch to \`sp-teams api\` CLI interop; do not pass workingDirectory unless the lead explicitly tells you to

## Message Protocol
When calling \`sp-teams api send-message\`, you MUST always include:
- from_worker: "<your-worker-name>" (your identity 鈥?check your inbox file for your worker name, never omit this)
...
Example:
sp-teams api send-message --input "{\"team_name\":\"${teamName}\",\"from_worker\":\"<your-worker-name>\",\"to_worker\":\"leader-fixed\",\"body\":\"ACK: <your-worker-name> initialized\"}" --json
...
1. Read: \`sp-teams api mailbox-list --input "{\"team_name\":\"${teamName}\",\"worker\":\"<your-worker-name>\"}" --json\`
2. Mark delivered: \`sp-teams api mailbox-mark-delivered --input "{\"team_name\":\"${teamName}\",\"worker\":\"<your-worker-name>\",\"message_id\":\"<MESSAGE_ID>\"}" --json\`
...
export async function writeTeamWorkerInstructionsFile(
  ...
  const outPath = join(specifyRuntimeStateDir(cwd), "team", teamName, "worker-agents.md");
...
${roleOverlay}`
  ...
  const outPath = join(specifyRuntimeStateDir(cwd), "team", teamName, "workers", workerName, "AGENTS.md");
  await mkdir(dirname(outPath), { recursive: true });
  await writeFile(outPath, composed);
  return outPath;
}
 ...
 * This is written to .specify/runtime/state/team/{team}/workers/{worker}/inbox.md by the lead.
 */
...
2. Send startup ACK to the lead mailbox BEFORE any task work (run this exact command):
   ...
   \`sp-teams api send-message --input "{\"team_name\":\"${teamName}\",\"from_worker\":\"${workerName}\",\"to_worker\":\"leader-fixed\",\"body\":\"ACK: ${workerName} initialized\"}" --json\`
...
7. Request a claim via CLI interop (\`sp-teams api claim-task --json\`) to claim it
8. Complete the work described in the task
9. After completing work, commit your changes before reporting completion:
   \`git add -A && git commit -m "task: <task-subject>"\`
   This ensures your changes are available for incremental integration into the leader branch.
10. Complete/fail it via lifecycle transition API (\`sp-teams api transition-task-status --json\`) from \`"in_progress"\` to \`"completed"\` or \`"failed"\` (include \`result\`/\`error\`)
11. Use \`sp-teams api release-task-claim --json\` only for rollback to \`pending\`
...
14. For legacy team_* MCP tools (hard-deprecated), use \`sp-teams api\`; do not pass \`workingDirectory\` unless the lead explicitly asks (if resolution fails, use leader cwd: \`${leaderCwd}\`)

## Mailbox Delivery Protocol (Required)
When you are notified about mailbox messages, always follow this exact flow:

1. List mailbox:
   \`sp-teams api mailbox-list --input "{\"team_name\":\"${teamName}\",\"worker\":\"${workerName}\"}" --json\`
2. For each undelivered message, mark delivery:
   \`sp-teams api mailbox-mark-delivered --input "{\"team_name\":\"${teamName}\",\"worker\":\"${workerName}\",\"message_id\":\"<MESSAGE_ID>\"}" --json\`
...
## Message Protocol
When using \`sp-teams api send-message\`, ALWAYS include from_worker with YOUR worker name:
- from_worker: "${workerName}"
- to_worker: "leader-fixed" (for leader) or "worker-N" (for peers)

Example: sp-teams api send-message --input "{\"team_name\":\"${teamName}\",\"from_worker\":\"${workerName}\",\"to_worker\":\"leader-fixed\",\"body\":\"ACK: initialized\"}" --json
...
3. Request a claim via CLI interop (\`sp-teams api claim-task --json\`)
4. Complete the work
5. After completing work, commit your changes before reporting completion:
   \`git add -A && git commit -m "task: <task-subject>"\`
   This ensures your changes are available for incremental integration into the leader branch.
6. Complete/fail via lifecycle transition API (\`sp-teams api transition-task-status --json\`) from \`"in_progress"\` to \`"completed"\` or \`"failed"\` (include \`result\`/\`error\`)
7. Use \`sp-teams api release-task-claim --json\` only for rollback to \`pending\`
...
export function generateTriggerMessage(
  workerName: string,
  teamName: string,
  teamStateRoot: string = DEFAULT_TEAM_STATE_ROOT_REFERENCE,
): string {
...
export function buildTriggerDirective(
  ...
  if (teamStateRoot !== DEFAULT_TEAM_STATE_ROOT_REFERENCE) {
    return {
...
export function buildMailboxTriggerDirective(
  ...
  return {
    intent: "pending-mailbox-review",
    text: `You have ${n} new message(s). Read ${mailboxPath}, act now, reply with progress, then continue assigned work or next feasible task.`,
  };
}
...
const DEFAULT_TEAM_STATE_ROOT_REFERENCE = ".specify/runtime/state";
