...
function isEmptyCherryPickConflict(result: CommandResult): boolean {
  const combined = `${result.stderr || ''}\n${result.stdout || ''}`.toLowerCase();
  return combined.includes('the previous cherry-pick is now empty')
  ...
  for (const worker of config.workers) {
    ...
    if (workerIsAheadOfLeader) {
      ...
      for (const commit of commits) {
        ...
        if (!pick.ok) {
          if (isEmptyCherryPickConflict(pick)) {
            const skipped = runGitCommand(repoRoot, ['cherry-pick', '--skip'], cwd);
            if (skipped.ok) {
              const currentLeaderHead = resolveLeaderHead(repoRoot, cwd) ?? leaderHead;
              state.last_integrated_head = commit;
              state.last_leader_head = currentLeaderHead;
              state.status = 'idle';
              state.conflict_commit = undefined;
              state.conflict_files = undefined;
              state.updated_at = new Date().toISOString();
              await appendIntegrationEvent(teamName, 'worker_cherry_pick_applied', worker, {
                ...
                commit,
                leader_head_before: leaderHead,
                leader_head_after: currentLeaderHead,
                worktree_path: worktreePath,
                summary: `skipped empty cherry-pick for ${worker.name} at ${commit.slice(0, 12)} because equivalent content already exists on leader`,
              }, cwd);
              commitHygieneEntries.push({
                recorded_at: new Date().toISOString(),
                ...
                task_id: worker.assigned_tasks[0],
                status: 'skipped',
                operational_commit: currentLeaderHead,
                source_commit: commit,
                leader_head_before: leaderHead,
                leader_head_after: currentLeaderHead,
                worktree_path: worktreePath,
                detail: 'Cherry-pick became empty because equivalent content already existed on leader; runtime skipped it without emitting conflict noise.',
              });
              continue;
            }
          }
...
function shouldRequireObservedStartupEvidence(
  requireWorkerStartupEvidence: boolean | undefined,
...
async function dispatchCriticalInboxInstruction(params: {
  ...
  const requiresObservedStartupEvidence = shouldRequireObservedStartupEvidence(
    requireWorkerStartupEvidence,
    workerCli,
  );
  ...
  if (receipt?.status === 'notified') {
    ...
    startupEvidence = await waitForWorkerStartupEvidence({
      teamName,
      workerName,
      workerCli: workerCli as TeamWorkerCli,
      cwd,
