...
function buildLaunchHints(
  ...
  if (mode === 'team') {
    return {
      shellCommand: `sp-teams ${recommendedHeadcount}:${fallbackRole} ${toQuotedCliArg(task)}`,
      skillCommand: `$sp-teams ${recommendedHeadcount}:${fallbackRole} ${toQuotedCliArg(task)}`,
...
function buildVerificationPlan(
  ...
  if (mode === 'team') {
    ...
    return {
      ...
      checkpoints: [
        'Launch via `sp-teams ...` (or `$sp-teams ...`) so the team runtime owns both parallel delivery and coordinated verification.',
