...
const SP_METADATA_SEGMENT_RE = /^\[SP(?:[#\]].*)?$/;
...
function isMetadataOnlyTmuxSegment(segment: string): boolean {
  return SP_METADATA_SEGMENT_RE.test(segment)
...
function isMetadataOnlyTmuxLine(line: string): boolean {
  ...
  const hasExplicitStatusSegment = segments.some((segment) => SP_METADATA_SEGMENT_RE.test(segment) || HUD_STATUS_SEGMENT_RE.test(segment));
...
function shouldUsePidFallback(): boolean {
  return process.env.SPECIFY_TMUX_PID_FALLBACK === "1";
}
