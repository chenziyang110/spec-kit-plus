...
describe("project wiki documentation contract", () => {
  it("documents the OMX-native storage, routing, and tool surface", () => {
    ...
    assert.match(doc, /`specify_wiki`/);
    assert.match(doc, /`\.specify\/runtime\/wiki\/`/);
    ...
    assert.match(doc, /These tools should operate only on `\.specify\/runtime\/wiki\/`/i);
  });
  ...
  it("keeps native hooks documentation aligned with the wiki lifecycle split", () => {
    ...
    assert.match(hooksDoc, /Storage.*`\.specify\/runtime\/wiki\/`/i);
