...
import { specifyWikiDir, specifyRuntimeRoot } from '../utils/paths.js';
import {
  type WikiLogEntry,
  type WikiPage,
...
export function getWikiDir(root: string): string {
  return specifyWikiDir(root);
}

export function ensureWikiDir(root: string): string {
  const wikiDir = getWikiDir(root);
  mkdirSync(wikiDir, { recursive: true });
  const runtimeRoot = specifyRuntimeRoot(root);
  mkdirSync(runtimeRoot, { recursive: true });
  const gitignorePath = join(runtimeRoot, '.gitignore');
