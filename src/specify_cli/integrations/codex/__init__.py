    ...
    def augment_generated_skills(
        ...
        self._augment_shared_skill(
            ...
            (
                "\n"
                f"## {agent_name} Subagents-First Dispatch\n\n"
                f"When running `sp-specify` in {agent_name}, use the fixed heavy discovery lifecycle and Codex native subagents.\n"
                "- The lifecycle is fixed to six stages: `intent-analysis`, `intent-confirmation`, `question-batch`, `batch-adversarial-review`, `completeness-audit`, and `final-handoff-decision`.\n"
                "- The fixed subagent roles are `intent-analyst`, `adversarial-reviewer`, and `completeness-auditor`.\n"
                "- The fixed domain order is `goal-and-users`, `triggers-and-primary-flow`, `boundaries-and-non-goals`, `failure-paths-exceptions-and-permissions`, `dependencies-constraints-and-upstream-downstream-impact`, and `acceptance-and-completeness-gap-closure`.\n"
                "- Use `spawn_agent` for bounded evidence lanes that support the active lifecycle stage without changing the fixed role set or domain order.\n"
                "- Launch all independent lanes in the current `parallel-subagents` wave before waiting.\n"
                "- Suggested bounded lanes include intent expansion against project context, adversarial challenge of the latest answered batch, and completeness auditing after all six domains have been processed.\n"
                "- Keep user-visible guidance aligned to the fixed-heavy contract; do not teach adaptive profiles, coverage modes, observer-style gates, or alternate lifecycle routing.\n"
                f"- Use `wait_agent` only at the documented join points between lifecycle stages, before domain closure, and before the final packet consistency pass.\n"
