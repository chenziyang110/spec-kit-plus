    ...
    def augment_generated_skills(
        ...
        self._augment_shared_skill(
            ...
            (
                "\n"
                f"## {agent_name} Subagents-First Dispatch\n\n"
                f"When running `sp-specify` in {agent_name}, use the brainstorming kernel and Codex native subagents.\n"
                ...
                "- Use `spawn_agent` for bounded evidence lanes that support the current lock step or compiled specification validation.\n"
                "- Launch all independent lanes in the current `parallel-subagents` wave before waiting.\n"
                "- Suggested bounded lanes include fact evidence gathering, route challenge, intent challenge, and compiled specification validation.\n"
                "- Keep user-visible guidance aligned to the brainstorming-kernel contract; do not teach chat-memory-only conclusions or silent downstream reinterpretation.\n"
                f"- Use `wait_agent` only at the documented join points before route closure, before complexity closure, and before the final packet consistency pass.\n"
