...
7. Before the first `TaskCreate`, compile an execution context bundle for the current batch:
   - run `{{specify-subcmd:specify-runtime cognition compass --intent implement --query="$ARGUMENTS" --format json}}` first and include the compass packet in teammate context
   ...
   - treat `expansion_ref` as a normal continuation path and run `{{specify-subcmd:specify-runtime cognition expand --id <id> --section <section> --format json}}` only when coverage state or live evidence requires more map detail
   ...
   - preserve advanced `lexicon -> semantic_intake -> query` / `specify-runtime cognition query --query-plan` only as conditional precision escalation when explicit concept decisions are needed or coverage cannot be resolved from the default compass packet
