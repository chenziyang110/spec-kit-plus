    ...
    def _append_planning_skill_cognition_refresh_guidance(
        ...
        *,
        ...
        addendum = (
            ...
            "- Use `/sp-map-update` for ordinary existing-baseline gaps. Use `/sp-map-scan` followed by `/sp-map-build` only for missing or unusable baseline, schema failure, zero active-generation `path_index` rows, `explicit_rebuild_requested`, or `baseline_identity_invalid`.\n"
        )
        return content + addendum
    ...
    def _append_runtime_project_cognition_gate(
        ...
        *,
        ...
        addendum = (
            ...
            f"{query_gate}\n"
            "- Interpret returned readiness: `ready` continues with the task-local bundle; `review` permits only returned `minimal_live_reads`; `ambiguous` asks the user to choose; `needs_update` treats map output as advisory, continues with live repository evidence, and recommends `{{invoke:map-update}}` as follow-up map maintenance; `needs_rebuild` treats map output as advisory, continues with live repository evidence, and recommends `{{invoke:map-scan}}`, then `{{invoke:map-build}}` only for missing or unusable baseline, schema failure, zero active-generation `path_index` rows, `explicit_rebuild_requested`, or `baseline_identity_invalid`; `blocked` reports the runtime issue as advisory map state and continues with live repository evidence unless the user explicitly requested map repair first.\n"
