    ...
    def _append_runtime_project_map_gate(
        ...
        *,
        ...
        addendum = (
            ...
            f"**Crucial First Step**: You MUST pass the runtime handbook contract first by reading `{'DEBUG-HANDBOOK.md' if command_name == 'debug' else 'BUILD-HANDBOOK.md'}` and the fixed chapter IDs required for {'debug' if command_name == 'debug' else 'this workflow'} {command_step}.\n"
            "- If the required runtime handbook is missing, stale, or too weak for the touched area, stop and tell the user to run `{{invoke:map-scan}}`, then `{{invoke:map-build}}`; wait for that refresh before continuing.\n"
            "- Treat the runtime atlas as handbook-only primary context; do not fall back to layered root/module atlas traversal for basic workflow routing.\n"
            "- Treat this as a hard gate, not a best-effort reminder; do not continue on chat memory or local instincts when runtime handbook coverage should be the source of truth.\n"
        )
    ...
    def _append_implement_leader_gate(
        ...
        *,
        ...
        gate_addendum = (
            ...
            "\n"
            "**Crucial First Step**: You MUST pass the runtime handbook contract first by reading `BUILD-HANDBOOK.md` and the fixed chapter IDs required for this workflow before any implementation actions.\n"
            "\n"
    ...
    def _augment_implement_skill(
        ...
        if marker not in content:
            addendum = (
                ...
                "- If subagent dispatch is unavailable or the packet is incomplete for the current batch, use `subagent-blocked`, record the blocker, and preserve the same join-point discipline.\n"
    ...
    def _augment_debug_skill(
        ...
        if gate_marker not in content:
            gate_addendum = (
                ...
                f"When running `sp-debug` in {agent_name}, you are the **leader**, not a freeform debugger.\n"
                "\n"
                "**Crucial First Step**: You MUST read `DEBUG-HANDBOOK.md` and the fixed chapter IDs required for debug before any investigation or fixes.\n"
                "\n"
                "Before applying fixes or running multiple independent investigation actions yourself:\n"
                "- Read the current debug session state and identify whether the investigation has two or more independent evidence-gathering lanes.\n"
    ...
    def _augment_quick_skill(
        ...
        if gate_marker not in content:
            gate_addendum = (
                ...
                f"When running `sp-quick` in {agent_name}, you are the **leader**, not the concrete implementer.\n"
                "\n"
                "**Crucial First Step**: You MUST read `BUILD-HANDBOOK.md` and the fixed chapter IDs required for this workflow before repository analysis or implementation.\n"
                "\n"
                "Before code edits, test edits, or implementation commands:\n"
                "- Read `.specify/memory/constitution.md` first if it exists.\n"
                "- Read `STATUS.md` for the active quick-task workspace, or create it if this quick task is new.\n"
