    ...
    def _augment_implement_teams_shared_contract(
        ...
        *,
        ...
        addendum = (
            ...
            "4. build a `query_plan` with `selected_concepts`, `rejected_concepts`, `concept_decisions`, `covered_facets`, `missing_facets`, `match_sources`, `lexicon_generation_id`, `expanded_queries`, and justified `paths`\n"
            "5. run `project-cognition query --intent implement --query-plan \"<query_plan_json>\" --format json` and preserve returned readiness, `minimal_live_reads`, and the task-local bundle in every teammate context packet\n"
            "6. if the query reports diagnostics, preserve `warnings`, `repair_hints`, normalized `query_plan`, structured `errors`, and `expected_shape` so the leader can repair the plan instead of losing the diagnostics in team chat\n\n"
