    ...
    def _append_delegation_surface_contract(
        ...
        *,
        ...
        if command_name == "implement":
            ...
            local_route = (
                "- Leader-inline is **not** docs-only / few-files / serial / time-saving. "
                "After `item-start` (`requires_worker: true`), spawn a subagent before any `write_scope` Edit/Write. "
                "`item-accept` requires matching `result submit`. "
                "Audited fallback only: `quick allow-inline --item Qn --reason \"spawn_failed: ...\"` "
                "(runtime rejects soft reasons).\n"
            )
        else:
            local_route = (
    ...
    def _append_quick_leader_gate(
        ...
        *,
        ...
        gate_addendum = (
            ...
            "\n"
            "### Hard Q Loop (mandatory; do not skip)\n"
            "1. `quick packet-compile --item Qn`\n"
            "2. `quick item-start --item Qn` (`requires_worker: true`)\n"
            "3. Spawn native subagent for write scope (**required**; no Edit/Write first)\n"
            "4. Join / wait\n"
            "5. `result submit --command quick --lane-id Qn --result-json '...'`\n"
            "6. `quick item-accept --item Qn --evidence '...'`\n"
            "7. Only then start the next Q\n"
            "\n"
            "**Illegal leader-inline excuses:** docs-only, few files, serial order, save time.\n"
            "**Legal leader-inline only via:** `quick allow-inline --item Qn --reason \"spawn_failed: ...\"`.\n"
            "\n"
            ...
            "- When `subagent-blocked`, run `quick allow-inline` with a concrete spawn/tool reason before any leader write_scope edit.\n"
            "\n"
            "**Hard rule:** Leader owns scope, STATUS, join, accept—not write_scope implementation.\n"
        )
    ...
    def _append_quick_routing_contract(
        ...
        *,
        ...
        addendum = (
            ...
            "- Dispatch shape: `one-subagent`, `parallel-subagents`, or `subagent-blocked`.\n"
            "- Execution surface: `native-subagents` by default; `leader-inline` only via `quick allow-inline` after spawn/tool failure.\n"
            "- Hard order per Q: `packet-compile` → `item-start` → spawn → join → `result submit --lane-id Qn` → `item-accept`.\n"
            "- Runtime refuses `item-accept` without a matching worker result (or prior allow-inline). Docs-only is invalid.\n"
            f"- Subagent dispatch: {descriptor.native_dispatch_hint}\n"
            f"- Integration-native join point: {descriptor.native_join_hint}\n"
            f"- Fallback path: {managed_team_hint}\n"
            "- Once the first lane is chosen, dispatch it before any leader write_scope Edit/Write.\n"
            ...
            f"- Decision order for {agent_name} `sp-quick`: mandatory packetized subagents → audited allow-inline only on spawn/tool failure → never silent leader-inline.\n"
            "- Require a validated `WorkerTaskPacket` (via `packet-compile`) before every Q dispatch.\n"
    ...
    def _augment_implement_skill(
        ...
        if marker not in content:
            addendum = (
                ...
                "\n"
                "For delegated waves:\n"
                f"- Use {agent_name}'s `native-subagents` lifecycle when available; prefer `execution_surface: native-subagents` when the current runtime supports it.\n"
                "- Fixed runtime budget: `max_parallel_subagents = 4` (at most four validated isolated lanes).\n"
                "- Prefer parallel only for multiple validated lanes with isolated write sets and an explicit join point.\n"
                f"{native_lifecycle}"
    ...
    def _augment_quick_skill(
        ...
        if gate_marker not in content:
            gate_addendum = (
                ...
                "\n"
                "### Hard Q Loop (mandatory; do not skip)\n"
                "For every in-progress `Qn` after checkpoint confirmation:\n"
                "1. `quick packet-compile --item Qn`\n"
                "2. `quick item-start --item Qn` (`requires_worker: true`)\n"
                f"3. `{dispatch_tool}` for that Qn write scope (**required**; no Edit/Write first)\n"
                f"4. `{join_tool}` / wait\n"
                "5. `result submit --command quick --workspace .planning/quick/<id>-<slug> --lane-id Qn --result-json '...'`\n"
                "6. `quick item-accept --item Qn --evidence '...'` (runtime requires matching worker result)\n"
                "7. Only then start the next Q\n"
                "\n"
                "**Forbidden after item-start / before item-accept:** Leader Edit/Write of that item's `write_scope`.\n"
                "**Illegal leader-inline excuses:** docs-only, few files, serial Q order, save time.\n"
                "**Legal leader-inline only via:** `quick allow-inline --item Qn --reason \"spawn_failed: ...\"` after real spawn/tool failure "
                "(runtime rejects soft reasons).\n"
                "\n"
                "Before code edits, test edits, or implementation commands:\n"
                "- Query `.specify/memory/constitution.md` first through `specify-runtime artifact show` if it exists.\n"
                ...
                "- Do not proceed to code edits, broad repository analysis, delegation, or validation commands until `understanding_confirmed: true` has been set by `quick checkpoint-confirm` / inherited stage (or an equivalent leased frontmatter patch).\n"
                "- After understanding is confirmed, run the Hard Q Loop for each Q item. Runtime rejects dependent starts before prerequisites are accepted and rejects `item-accept` without a worker result.\n"
                "- Dispatch `one-subagent` for serial/shared write scopes; `parallel-subagents` only for isolated write sets.\n"
                ...
                "- When `subagent-blocked` is used, you **MUST** run `quick allow-inline` with a concrete spawn/tool reason before any leader write_scope edit.\n"
                "\n"
        ...
        addendum = (
            ...
            "- Decision Checkpoint: before dispatch, stage/show the runtime Decision Checkpoint and Delivery Map (`quick checkpoint-stage` / `checkpoint-show`). Confirm only user-owned Q1/Q2 deliverables, dependencies, and acceptance; never ask users to approve subagent/batch/file choreography.\n"
            "- Hard order per Q: `packet-compile` → `item-start` → "
            f"`{dispatch_tool}` → `{join_tool}` → `result submit --lane-id Qn` → `item-accept`.\n"
            "- Runtime refuses `item-accept` without a matching worker result (or prior `quick allow-inline`).\n"
            "- Docs-only, few files, serial Q order, and time-saving do **not** authorize leader-inline.\n"
            "- Dispatch `one-subagent` or `parallel-subagents` only after the Decision Checkpoint is confirmed or inherited.\n"
            "- Use `subagent-blocked` when the selected native lane cannot proceed and no authorized safe route remains; do not require or invoke `managed-team`/`sp-teams` unless durable execution was explicitly selected. Run `quick allow-inline --reason` before any leader write_scope edit.\n"
            f"- Use `{dispatch_tool}` for every substantive Q write_scope lane.\n"
            "- Once the first lane is chosen, dispatch it before any leader write_scope Edit/Write.\n"
            "- If multiple safe subagent lanes exist and they materially improve throughput, dispatch them in parallel.\n"
