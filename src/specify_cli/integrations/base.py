    ...
    def _append_quick_leader_gate(
        ...
        *,
        ...
        gate_addendum = (
            ...
            "- If the selected strategy is `native-multi-agent`, you **MUST** dispatch the concrete work through the current runtime's native worker lanes before considering any fallback path.\n"
            "- If the selected strategy is `single-lane`, you **MUST** dispatch exactly one delegated worker lane before considering any leader-local fallback.\n"
    ...
    def _append_quick_routing_contract(
        ...
        *,
        ...
        addendum = (
            ...
            "- Interpret `single-lane` as one delegated worker lane, not leader self-execution.\n"
            "- Treat legacy `single-agent` state as a compatibility alias for the same delegated single-lane path.\n"
    ...
    def _augment_implement_skill(
        ...
        if marker not in content:
            addendum = (
                ...
                "- Do **not** ask the user whether the `single-lane` batch should switch to delegated execution; dispatch the lane by default and only discuss fallback after delegation surfaces concretely fail.\n"
    ...
    def _augment_implement_teams_shared_contract(
        ...
        *,
        ...
        addendum = (
            ...
            "3. for implementation-oriented teams flows, preserve the user-visible strategy names `single-lane`, `native-multi-agent`, `sidecar-runtime`, while still accepting legacy `single-agent` state as a compatibility alias for `single-lane`\n"
