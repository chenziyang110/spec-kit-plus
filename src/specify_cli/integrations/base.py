    ...
    * ``context_file``     — path (relative to project root) of the agent
    ...
    def _append_specify_pre_analysis_protocol(
        ...
        *,
        ...
        addendum = (
            ...
            "- Run project cognition planning navigation with `project-cognition lexicon --intent plan --mode catalog`, use the alias catalog to write `semantic_intake` with `normalized_query`, `intent_facets`, `negative_constraints`, and `alias_interpretations`, carry `lexicon_generation_id`, write `concept_decisions` with `covered_facets`, `missing_facets`, and `match_sources`, add `repository_search_terms`, then run `project-cognition query --intent plan --query-plan`; facet coverage must drive selection, not top similarity alone. Do not search only the raw user words; use project-language search terms before broad repository search. Returned `minimal_live_reads` feeds the coverage-model check.\n"
    ...
    def _project_cognition_query_gate_line(self, *, command_name: str, command_step: str) -> str:
        ...
        return (
            ...
            f"{command_step}. Candidate selection must satisfy facet coverage; do not trust top similarity alone. Do not search only the raw user words; use project-language search terms before broad repository search. Use the returned readiness, task-local bundle, and `minimal_live_reads`."
        )
    ...
    def _augment_implement_teams_shared_contract(
        ...
        *,
        ...
        addendum = (
            ...
            "1. run `project-cognition lexicon --intent implement --mode catalog` and include the lexicon result and alias catalog in the execution context bundle\n"
            "2. write a `semantic_intake` object with `workflow_intent`, `normalized_query`, `intent_facets`, `negative_constraints`, `alias_interpretations`, and `open_semantic_questions`\n"
