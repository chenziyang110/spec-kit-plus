...
def _render_spec_kit_managed_block(*, newline: str) -> str:
    return newline.join(
        [
            ...
            '- Reserve `sp-map-scan` followed by `sp-map-build` for missing baselines or explicit full rebuild cases; when a full refresh can be completed now, use `specify project-cognition complete-refresh --format json` as the successful-refresh finalizer, otherwise use `specify project-cognition mark-dirty --reason "<reason>" --format json` as the manual override/fallback.',
