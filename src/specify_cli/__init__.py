...
def project_cognition_update_command(
    changed_paths: Optional[list[str]] = typer.Option(
        None,
        "--changed-paths",
        "--scope",
        help="Changed path; may be specified more than once. If omitted, derive paths from git diff/status.",
    ),
...
def init(
    ...
    enhancement_lines.extend(
        [
            ...
            f"○ [cyan]{_display_cmd('analyze')}[/] [bright_black](optional diagnostic / legacy revalidation)[/bright_black] - Cross-artifact consistency & alignment report, including boundary guardrail drift (after [cyan]{_display_cmd('tasks')}[/], before [cyan]{_display_cmd('implement')}[/])",
