...
def _render_project_map_preflight_guidance(
    ...
    console.print(
        "[yellow]Project cognition output is advisory; continue with live evidence and refresh the map only when the workflow requires it.[/yellow]"
    )
    ...
    if state == "missing_baseline":
        console.print(
            "Run [cyan]/sp-map-scan[/cyan], then [cyan]/sp-map-build[/cyan] to create the graph-native project cognition baseline before relying on map output."
        )
    elif state == "support_drift":
        console.print(
            "Resolve, commit, or intentionally ignore the support-surface drift before relying on refreshed map output."
        )
    ...
    *,
    ...
    if freshness != "fresh":
        ...
        if freshness == "runtime_stale" and readiness == "review":
            console.print(
                "[yellow]Warning:[/yellow] Runtime stale state is review-only here; continue only if the current task is still local and the live evidence supports it."
            )
...
def _render_spec_kit_managed_block(*, project_root: Path, newline: str) -> str:
    ...
    template = newline.join(
        [
            ...
            "- Run `{{specify-subcmd:specify-runtime learning start --command <workflow> --format json}}` before non-trivial decisions that depend on local conventions, constraints, or past lessons; expand only selected matching Learning through `show_argv`.",
            "",
            ...
            "",
            "- Treat `{{specify-subcmd:specify-runtime api list --format json}}` as the authoritative agent-facing CLI surface; use `{{specify-subcmd:specify-runtime api show workflow.block --format json}}` and `{{specify-subcmd:specify-runtime api schema workflow-block-input --format json}}` only for the blocked-exit contract.",
            ...
            "- Do not invent unsupported CLI names such as `specify-runtime create-feature`.",
...
def _install_shared_infra(
    ...
    *,
    ...
    cognition_dir = project_path / ".specify" / "project-cognition"
        ...
        console.print(
            _open_block(
                ...
                [
                    ...
                    "",
                    "[dim]Install the prebuilt specify-runtime release binary manually or set SPECIFY_RUNTIME_BIN as a one-time bootstrap seed, then rerun init so the project-local .specify/bin entrypoint is provisioned.[/dim]",
                ],
                accent="yellow",
            )
        )
